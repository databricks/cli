# Code generated by pydabs-codegen. DO NOT EDIT.

from dataclasses import dataclass
from typing import TYPE_CHECKING, TypedDict

from databricks.bundles.core._transform import _transform
from databricks.bundles.core._transform_to_json import _transform_to_json_value
from databricks.bundles.core._variable import VariableOr

if TYPE_CHECKING:
    from typing_extensions import Self


@dataclass(kw_only=True)
class UnityCatalog:
    """
    Unity Catalog Destinations for OTEL telemetry export.
    """

    logs_table: VariableOr[str]
    """
    [Public Preview] Unity Catalog table for OTEL logs.
    """

    metrics_table: VariableOr[str]
    """
    [Public Preview] Unity Catalog table for OTEL metrics.
    """

    traces_table: VariableOr[str]
    """
    [Public Preview] Unity Catalog table for OTEL traces (spans).
    """

    @classmethod
    def from_dict(cls, value: "UnityCatalogDict") -> "Self":
        return _transform(cls, value)

    def as_dict(self) -> "UnityCatalogDict":
        return _transform_to_json_value(self)  # type:ignore


class UnityCatalogDict(TypedDict, total=False):
    """"""

    logs_table: VariableOr[str]
    """
    [Public Preview] Unity Catalog table for OTEL logs.
    """

    metrics_table: VariableOr[str]
    """
    [Public Preview] Unity Catalog table for OTEL metrics.
    """

    traces_table: VariableOr[str]
    """
    [Public Preview] Unity Catalog table for OTEL traces (spans).
    """


UnityCatalogParam = UnityCatalogDict | UnityCatalog
