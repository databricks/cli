...
func BuildStateFromTF(
	...
	for _, node := range nodes {
		...
		// Compact hashed_in_state fields (e.g. serialized_dashboard) to a content hash
		// before persisting, matching what the direct engine writes on a normal deploy.
		compacted, err := dresources.CompactState(adapter.ResourceConfig(), sv.Value)
		if err != nil {
			return fmt.Errorf("%s: compacting state: %w", node, err)
		}
