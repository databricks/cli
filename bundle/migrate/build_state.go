...
func BuildStateFromTF(
	...
	for _, pat := range patterns {
		...
		if err != nil {
			return warningsSeen, warnSafeErr, err
		}
	}
	...
	for _, node := range nodes {
		...
		if group == "" {
			return warningsSeen, warnSafeErr, safeerr.Errorf("cannot determine resource type for %q", key)
		}
		...
		if !ok {
			...
			setWarnSafeErr(&warnSafeErr, safeerr.Errorf("unsupported resource type %q for %s, skipping", safeerr.Safe(group), key))
			continue
		}

		inputConfig, err := configRoot.GetResourceConfig(node)
		if err != nil {
			return warningsSeen, warnSafeErr, safeerr.Errorf("%s: getting config: %w", key, err)
		}

		inputSV, err := adapter.PrepareInputConfig(inputConfig, node)
		if err != nil {
			return warningsSeen, warnSafeErr, safeerr.Errorf("%s: PrepareInputConfig: %w", key, err)
		}

		newStateValue, err := adapter.PrepareState(inputSV.Value)
		if err != nil {
			return warningsSeen, warnSafeErr, safeerr.Errorf("%s: PrepareState: %w", key, err)
		}

		refs, err := direct.ExtractReferences(configRoot.Value(), node, adapter.StateType())
		if err != nil {
			return warningsSeen, warnSafeErr, safeerr.Errorf("%s: extracting references: %w", key, err)
		}
		...
		if _, ok := sv.Refs["object_id"]; ok {
			if err := structaccess.Set(sv.Value, structpath.NewStringKey(nil, "object_id"), id); err != nil {
				return warningsSeen, warnSafeErr, safeerr.Errorf("%s: setting object_id: %w", key, err)
			}
		...
		for _, pending := range pendingRefs {
			...
			if err != nil {
				return warningsSeen, warnSafeErr, safeerr.Errorf("%s: parsing field path %q: %w", key, pending.fieldPathStr, err)
			}
			...
			if err != nil {
				return warningsSeen, warnSafeErr, safeerr.Errorf("%s: cannot resolve field %q (template %q): %w", key, pending.fieldPathStr, pending.refTemplate, err)
			}
			if warned {
				warningsSeen = true
				// The disagreeing values are user data; the resource type and the
				// stage are not.
				setWarnSafeErr(&warnSafeErr, safeerr.Errorf(
			...
			if err := structaccess.Set(sv.Value, fieldPath, value); err != nil {
				return warningsSeen, warnSafeErr, safeerr.Errorf("%s: cannot set resolved value for field %q: %w", key, pending.fieldPathStr, err)
			}
			delete(sv.Refs, pending.fieldPathStr)
		}

		if len(sv.Refs) > 0 {
			return warningsSeen, warnSafeErr, safeerr.Errorf("%s: unresolved references: %v", key, sv.Refs)
		}
		...
		if v, err := LookupTFField(tfAttrs, group, srcName, structpath.NewStringKey(nil, "etag")); err == nil {
			if etag, ok := v.(string); ok && etag != "" {
				if err := structaccess.Set(sv.Value, structpath.NewStringKey(nil, "etag"), etag); err != nil {
					return warningsSeen, warnSafeErr, safeerr.Errorf("%s: cannot set etag: %w", key, err)
				}
			}
		}

		if err := stateDB.SaveState(ctx, node, id, sv.Value, dependsOn); err != nil {
			return warningsSeen, warnSafeErr, safeerr.Errorf("%s: SaveState: %w", key, err)
		}
	}

	return warningsSeen, warnSafeErr, nil
}

// setWarnSafeErr records err's message template in target unless one is already
// there: the first warning is the one reported, matching how the first error
// diagnostic is the one a deploy reports.
func setWarnSafeErr(target *string, err error) {
	if *target == "" {
