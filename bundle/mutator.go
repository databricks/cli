...
// rather than turning it into a diagnostic. The mutator machinery still runs, so
// typed config changes are synced back to the dynamic tree. A failure to sync is
// logged as a diagnostic, not returned.
