...
func (d *Deployer) Plan(ctx context.Context, client *databricks.WorkspaceClient, db *tnstate.TerranovaState, settings ResourceSettings, inputConfig any, localOnly, refresh bool) (deployplan.ActionType, error) {
	result, err := d.plan(ctx, client, db, settings, inputConfig, localOnly, refresh)
	if err != nil {
...
func (d *Deployer) plan(ctx context.Context, client *databricks.WorkspaceClient, db *tnstate.TerranovaState, settings ResourceSettings, inputConfig any, localOnly, refresh bool) (deployplan.ActionType, error) {
	resource, cfgType, err := New(client, d.Group, d.Key, inputConfig)
	if err != nil {
		return "", err
	}
	d.Resource = resource
	...
	actionType, err := calcDiff(settings, resource, savedState, config)
	if err != nil {
		return "", err
	}

	switch actionType {
	// For these actions no need to check remote state
...
func (d *Deployer) refreshRemoteState(ctx context.Context, id string) error {
	if d.Resource == nil {
		return errors.New("internal error: resource not initialized")
	}
...
func (d *Deployer) ResolveReferenceLocalOrRemote(ctx context.Context, db *tnstate.TerranovaState, reference string, actionType deployplan.ActionType, config any, remoteType reflect.Type) (any, error) {
	...
	remoteValidErr := structaccess.Validate(remoteType, fieldPath)
...
func (d *Deployer) ResolveReferenceRemote(ctx context.Context, db *tnstate.TerranovaState, reference string) (any, error) {
	path, ok := dynvar.PureReferenceToPath(reference)
	if !ok || len(path) <= 3 || path[0].Key() != "resources" || path[1].Key() != d.Group || path[2].Key() != d.Key {
		return nil, fmt.Errorf("internal error: expected reference to resources.%s.%s, got %q", d.Group, d.Key, reference)
	}
...
func (d *Deployer) ReadRemoteStateField(ctx context.Context, db *tnstate.TerranovaState, fieldPath dyn.Path) (any, error) {
	// We have options:
