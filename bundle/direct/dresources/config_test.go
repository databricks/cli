...
func TestConfigFiles(t *testing.T) {
	...
	for _, name := range names {
		resourceType := strings.TrimSuffix(strings.TrimSuffix(path.Base(name), ymlSuffix), generatedSuffix)
...
// hand-written configs/<resource_type>.yml files: duplicate field entries within the
// same category of a resource, and entries that the autogenerated
// configs/<resource_type>.generated.yml already produces from the OpenAPI schema.
	...
	for resourceType, rc := range handWritten.Resources {
		...
		for _, c := range categoryRules(rc) {
			...
			for _, r := range c.rules {
				...
				if seen[field] {
					t.Errorf("bundle/direct/dresources/configs/%s.yml: %s lists %q twice; remove the duplicate entry", resourceType, c.name, field)
				}
				seen[field] = true
				if genFields[c.name][field] {
					t.Errorf("bundle/direct/dresources/configs/%[1]s.yml: %[2]s entry %[3]q is already produced by %[1]s.generated.yml; remove it", resourceType, c.name, field)
				}
			}
		}
	}
...
// in configs/<resource_type>.yml do not duplicate the automatic missing-in-remote suppression. A field
...
func TestResourcesYMLNoRedundantMissingInRemote(t *testing.T) {
	...
	for resourceType, rc := range cfg.Resources {
		...
		if err != nil {
			t.Errorf("%s.yml: failed to create adapter: %v", resourceType, err)
			continue
		}
		for _, r := range rc.IgnoreRemoteChanges {
			...
			if inState && !inRemote {
				t.Errorf("%s.yml: ignore_remote_changes entry %q is automatically handled (field absent from RemoteType); remove it", resourceType, r.Field)
			}
		}
	}
}
		...
		for _, c := range actionCats {
			for _, r := range c.rules {
				...
				if prev, ok := firstCat[field]; ok {
					t.Errorf("bundle/direct/dresources/configs/%s.yml: %q is listed in both %s and %s; a field's action belongs to exactly one category", resourceType, field, prev, c.name)
				} else {
