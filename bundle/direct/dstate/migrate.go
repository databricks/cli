...
// ("grants" key for the slice) to the new format ("_" key for EmbeddedSlice).
...
// oldGrantsStateV2 is the old format with "grants" key.
...
func migrateGrantsEntry(raw json.RawMessage) (json.RawMessage, error) {
	...
	// If old format had no grants, might already be migrated.
	if len(old.Grants) == 0 {
		return raw, nil
	}

	// Re-serialize with "_" key instead of "grants".
	type newFormat struct {
		SecurableType string            `json:"securable_type"`
		FullName      string            `json:"full_name"`
		EmbeddedSlice []json.RawMessage `json:"_,omitempty"`
	}
