...
func TestShouldSkipNormalized(t *testing.T) {
	...
	tests := []struct {
		...
		{"slash rule, slash-only diff", "storage_root", "s3://b/x/", "s3://b/x", "slash", true},
		{"slash rule stays case-sensitive", "storage_root", "s3://b/X", "s3://b/x", "", false},
		{"case rule, case-only diff", "channel", "current", "CURRENT", "case", true},
		{"case rule, genuine diff", "channel", "current", "preview", "", false},
		{"both rules, slash-only diff", "weird", "Foo/", "Foo", "slash", true},
		{"both rules, case-only diff", "weird", "PRO", "pro", "case", true},
		{"both rules, slash and case diff", "weird", "Foo/", "foo", "case", true},
		{"both rules, genuine diff", "weird", "foo", "bar", "", false},
		{"non-string change", "channel", 1, 2, "", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			path, err := structpath.ParsePath(tt.path)
