...
func parseResourcesState(ctx context.Context, path string) (ExportedResourcesMap, error) {
	...
	for _, resource := range state.Resources {
		...
		for _, instance := range resource.Instances {
			...
			case "apps", "database_instances", "database_catalogs", "synced_database_tables", "postgres_projects", "postgres_branches", "postgres_endpoints", "postgres_catalogs", "postgres_synced_tables", "postgres_roles":
				resourceKey = "resources." + groupName + "." + resource.Name
				resourceState = ResourceState{ID: instance.Attributes.Name}
			case "dashboards":
				resourceKey = "resources." + groupName + "." + resource.Name
