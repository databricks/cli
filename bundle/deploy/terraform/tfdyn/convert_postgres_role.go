...
type postgresRoleConverter struct{}

func (c postgresRoleConverter) Convert(ctx context.Context, key string, vin dyn.Value, out *schema.Resources) error {
	...
	vout, diags := convert.Normalize(schema.ResourcePostgresRole{}, vout)
	for _, diag := range diags {
		log.Debugf(ctx, "postgres role normalization diagnostic: %s", diag.Summary)
	}
	...
	out.PostgresRole[key] = vout.AsAny()

	return nil
}

func init() {
	registerConverter("postgres_roles", postgresRoleConverter{})
}
