...
- - `notification`
  - Map
  - User or Notification Destination to notify when alert is triggered. See [\_](#alertsnameevaluationnotification).
  
- - `source`
  - Map
  - Source column from result to use to evaluate alert. See [\_](#alertsnameevaluationsource).
  
- - `threshold`
  - Map
  - Threshold to user for alert evaluation, can be a column or a value. See [\_](#alertsnameevaluationthreshold).
...
- - `subscriptions`
  - Sequence
  - See [\_](#alertsnameevaluationnotificationsubscriptions).
...
- - `column`
  - Map
  - See [\_](#alertsnameevaluationthresholdcolumn).
  
- - `value`
  - Map
  - See [\_](#alertsnameevaluationthresholdvalue).
...
- - `telemetry_export_destinations`
  - Sequence
  - See [\_](#appsnametelemetry_export_destinations).
...
- - `custom_tags`
  - Sequence
  - Custom tags associated with the instance. This field is only included on create and update responses. See [\_](#database_instancesnamecustom_tags).
...
- - `parent_instance_ref`
  - Map
  - The ref of the parent instance. This is only available if the instance is child instance. Input: For specifying the parent instance to create a child instance. Optional. Output: Only populated if provided as input to create a child instance. See [\_](#database_instancesnameparent_instance_ref).
...
- - `alert_task`
  - Map
  - The task evaluates a Databricks alert and sends notifications to subscribers when the `alert_task` field is present. See [\_](#jobsnametasksalert_task).
...
- - `compute`
  - Map
  - Task level compute configuration. See [\_](#jobsnametaskscompute).
...
- - `subscribers`
  - Sequence
  - The subscribers receive alert evaluation result notifications after the alert task is completed. The number of subscriptions is limited to 100. See [\_](#jobsnametasksalert_tasksubscribers).
...
- - `environment`
  - Map
  - Environment specification for this pipeline used to install dependencies. See [\_](#pipelinesnameenvironment).
...
- - `source_configurations`
  - Sequence
  - Top-level source configurations. See [\_](#pipelinesnameingestion_definitionsource_configurations).
...
- - `query_based_connector_config`
  - Map
  - Configurations that are only applicable for query-based ingestion connectors. See [\_](#pipelinesnameingestion_definitionobjectsreporttable_configurationquery_based_connector_config).
...
- - `connector_options`
  - Map
  - (Optional) Source Specific Connector Options. See [\_](#pipelinesnameingestion_definitionobjectsschemaconnector_options).
...
- - `jira_options`
  - Map
  - Jira specific options for ingestion. See [\_](#pipelinesnameingestion_definitionobjectsschemaconnector_optionsjira_options).
  
- - `meta_ads_options`
  - Map
  - Meta Marketing (Meta Ads) specific options for ingestion. See [\_](#pipelinesnameingestion_definitionobjectsschemaconnector_optionsmeta_ads_options).
...
- - `custom_insights_lookback_window`
  - Integer
  - (Optional) Window in days to revisit data during sync to capture updated conversion data from the API.
...
- - `start_date`
  - String
  - (Optional) Start date in yyyy-MM-dd format (e.g. 2025-01-15). Data added after this date will be ingested
...
- - `query_based_connector_config`
  - Map
  - Configurations that are only applicable for query-based ingestion connectors. See [\_](#pipelinesnameingestion_definitionobjectsschematable_configurationquery_based_connector_config).
...
- - `connector_options`
  - Map
  - (Optional) Source Specific Connector Options. See [\_](#pipelinesnameingestion_definitionobjectstableconnector_options).
...
- - `confluence_options`
  - Map
  - Confluence specific options for ingestion. See [\_](#pipelinesnameingestion_definitionobjectstableconnector_optionsconfluence_options).
...
- - `meta_ads_options`
  - Map
  - Meta Marketing (Meta Ads) specific options for ingestion. See [\_](#pipelinesnameingestion_definitionobjectstableconnector_optionsmeta_ads_options).
...
- - `query_based_connector_config`
  - Map
  - Configurations that are only applicable for query-based ingestion connectors. See [\_](#pipelinesnameingestion_definitionobjectstabletable_configurationquery_based_connector_config).
...
- - `catalog`
  - Map
  - Catalog-level source configuration parameters. See [\_](#pipelinesnameingestion_definitionsource_configurationscatalog).
...
- - `postgres`
  - Map
  - Postgres-specific catalog-level configuration parameters. See [\_](#pipelinesnameingestion_definitionsource_configurationscatalogpostgres).
...
- - `slot_config`
  - Map
  - Optional. The Postgres slot configuration to use for logical replication. See [\_](#pipelinesnameingestion_definitionsource_configurationscatalogpostgresslot_config).
...
- - `query_based_connector_config`
  - Map
  - Configurations that are only applicable for query-based ingestion connectors. See [\_](#pipelinesnameingestion_definitiontable_configurationquery_based_connector_config).
...
- - `glob`
  - Map
  - The unified field to include source codes. Each entry can be a notebook path, a file path, or a folder path that ends `/**`. This field cannot be used together with `notebook` or `file`. See [\_](#pipelinesnamelibrariesglob).
...
- - `lifecycle`
  - Map
  - See [\_](#postgres_databasesnamelifecycle).
...
### postgres_databases._name_.lifecycle
...
- - `attributes`
  - Map
  - The desired API-exposed Postgres role attributes to associate with the role. See [\_](#postgres_rolesnameattributes).
  
- - `auth_method`
  - String
  - How the role is authenticated when connecting to Postgres. If left unspecified, a meaningful authentication method is derived from the identity_type.
...
- - `lifecycle`
  - Map
  - See [\_](#postgres_rolesnamelifecycle).
...
- - `postgres_role`
  - String
  - The name of the Postgres role. Required when creating the role.
...
### postgres_roles._name_.lifecycle
...
- - `spec`
  - Map
  - Specification of a synced database table. See [\_](#synced_database_tablesnamespec).
