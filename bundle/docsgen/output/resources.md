...
- - `child_instance_refs`
  - Sequence
  - The refs of the child instances. This is only available if the instance is parent instance. See [\_](#database_instancesnamechild_instance_refs).
...
- - `effective_enable_pg_native_login`
  - Boolean
  - xref AIP-129. `enable_pg_native_login` is owned by the client, while `effective_enable_pg_native_login` is owned by the server. `enable_pg_native_login` will only be set in Create/Update response messages if and only if the user provides the field via the request. `effective_enable_pg_native_login` on the other hand will always bet set in all response messages (Create/Update/Get/List).
...
- - `effective_node_count`
  - Integer
  - xref AIP-129. `node_count` is owned by the client, while `effective_node_count` is owned by the server. `node_count` will only be set in Create/Update response messages if and only if the user provides the field via the request. `effective_node_count` on the other hand will always bet set in all response messages (Create/Update/Get/List).
  
- - `effective_retention_window_in_days`
  - Integer
  - xref AIP-129. `retention_window_in_days` is owned by the client, while `effective_retention_window_in_days` is owned by the server. `retention_window_in_days` will only be set in Create/Update response messages if and only if the user provides the field via the request. `effective_retention_window_in_days` on the other hand will always bet set in all response messages (Create/Update/Get/List).
...
### database_instances._name_.child_instance_refs
...
- - `spec`
  - Map
  - The environment entity used to preserve serverless environment side panel, jobs' environment for non-notebook task, and DLT's environment for classic and serverless pipelines. In this minimal environment spec, only pip dependencies are supported. See [\_](#jobsnameenvironmentsspec).
...
- - `environment_version`
  - String
  - Required. Environment version used by the environment. Each version comes with a specific Python version and a set of Python packages. The version is a string, consisting of an integer.
...
- - `dbt_platform_task`
  - Map
  - See [\_](#jobsnametasksdbt_platform_task).
...
### jobs._name_.tasks.dbt_platform_task
...
- - `data_classification_config`
  - Map
  - [Create:OPT Update:OPT] Data classification related config. See [\_](#quality_monitorsnamedata_classification_config).
...
- - `on_new_classification_tag_detected`
  - Map
  - Destinations to send notifications on new classification tag detected. See [\_](#quality_monitorsnamenotificationson_new_classification_tag_detected).
...
- - `auto_stop_mins`
  - Integer
  - The amount of time in minutes that a SQL warehouse must be idle (i.e., no RUNNING queries) before it is automatically stopped.  Supported values:   - Must be >= 0 mins for serverless warehouses   - Must be == 0 or >= 10 mins for non-serverless warehouses  - 0 indicates no autostop.  Defaults to 120 mins
...
- - `data_synchronization_status`
  - Map
  - Status of a synced table. See [\_](#synced_database_tablesnamedata_synchronization_status).
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#synced_database_tablesnamelifecycle).
...
### synced_database_tables._name_.data_synchronization_status
...
- - `continuous_update_status`
  - Map
  - Detailed status of a synced table. Shown if the synced table is in the SYNCED_CONTINUOUS_UPDATE or the SYNCED_UPDATING_PIPELINE_RESOURCES state. See [\_](#synced_database_tablesnamedata_synchronization_statuscontinuous_update_status).
...
- - `provisioning_status`
  - Map
  - Detailed status of a synced table. Shown if the synced table is in the PROVISIONING_PIPELINE_RESOURCES or the PROVISIONING_INITIAL_SNAPSHOT state. See [\_](#synced_database_tablesnamedata_synchronization_statusprovisioning_status).
  
- - `triggered_update_status`
  - Map
  - Detailed status of a synced table. Shown if the synced table is in the SYNCED_TRIGGERED_UPDATE or the SYNCED_NO_PENDING_UPDATE state. See [\_](#synced_database_tablesnamedata_synchronization_statustriggered_update_status).
  
:::
  
  
### synced_database_tables._name_.data_synchronization_status.continuous_update_status
...
- - `initial_pipeline_sync_progress`
  - Map
  - Progress of the initial data synchronization. See [\_](#synced_database_tablesnamedata_synchronization_statuscontinuous_update_statusinitial_pipeline_sync_progress).
...
### synced_database_tables._name_.data_synchronization_status.continuous_update_status.initial_pipeline_sync_progress
...
### synced_database_tables._name_.data_synchronization_status.failed_status
...
### synced_database_tables._name_.data_synchronization_status.last_sync
...
- - `delta_table_sync_info`
  - Map
  - See [\_](#synced_database_tablesnamedata_synchronization_statuslast_syncdelta_table_sync_info).
...
### synced_database_tables._name_.data_synchronization_status.last_sync.delta_table_sync_info
...
### synced_database_tables._name_.data_synchronization_status.provisioning_status
...
- - `initial_pipeline_sync_progress`
  - Map
  - Details about initial data synchronization. Only populated when in the PROVISIONING_INITIAL_SNAPSHOT state. See [\_](#synced_database_tablesnamedata_synchronization_statusprovisioning_statusinitial_pipeline_sync_progress).
  
:::
  
  
### synced_database_tables._name_.data_synchronization_status.provisioning_status.initial_pipeline_sync_progress
...
### synced_database_tables._name_.data_synchronization_status.triggered_update_status
...
- - `triggered_update_progress`
  - Map
  - Progress of the active data synchronization pipeline. See [\_](#synced_database_tablesnamedata_synchronization_statustriggered_update_statustriggered_update_progress).
  
:::
  
  
### synced_database_tables._name_.data_synchronization_status.triggered_update_status.triggered_update_progress
