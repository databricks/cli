...
- - `evaluation`
  - Map
  - [Public Preview]. See [\_](#alertsnameevaluation).
...
- - `run_as`
  - Map
  - [Public Preview]. See [\_](#alertsnamerun_as).
...
- - `schedule`
  - Map
  - [Public Preview]. See [\_](#alertsnameschedule).
...
- - `comparison_operator`
  - String
  - [Public Preview] Operator used for comparison in alert evaluation.
  
- - `empty_result_state`
  - String
  - [Public Preview] Alert state if result is empty. Please avoid setting this field to be `UNKNOWN` because `UNKNOWN` state is planned to be deprecated.
  
- - `notification`
  - Map
  - [Public Preview] User or Notification Destination to notify when alert is triggered. See [\_](#alertsnameevaluationnotification).
  
- - `source`
  - Map
  - [Public Preview] Source column from result to use to evaluate alert. See [\_](#alertsnameevaluationsource).
  
- - `threshold`
  - Map
  - [Public Preview] Threshold to user for alert evaluation, can be a column or a value. See [\_](#alertsnameevaluationthreshold).
  
:::
  
  
### alerts._name_.evaluation.notification
  
**`Type: Map`**
  
[Public Preview] User or Notification Destination to notify when alert is triggered.
...
### alerts._name_.evaluation.source
  
**`Type: Map`**
  
[Public Preview] Source column from result to use to evaluate alert
...
### alerts._name_.evaluation.threshold
  
**`Type: Map`**
  
[Public Preview] Threshold to user for alert evaluation, can be a column or a value.
...
- - `value`
  - Map
  - [Public Preview]. See [\_](#alertsnameevaluationthresholdvalue).
  
:::
  
  
### alerts._name_.evaluation.threshold.column
...
### alerts._name_.evaluation.threshold.value
...
- - `service_principal_name`
  - String
  - [Public Preview] Application ID of an active service principal. Setting this field requires the `servicePrincipal/user` role.
  
- - `user_name`
  - String
  - [Public Preview] The email of an active workspace user. Can only set this field to their own email.
  
:::
  
  
### alerts._name_.schedule
...
- - `pause_status`
  - String
  - [Public Preview] Indicate whether this schedule is paused or not.
  
- - `quartz_cron_schedule`
  - String
  - [Public Preview] A cron expression using quartz syntax that specifies the schedule for this pipeline. Should use the quartz format described here: http://www.quartz-scheduler.org/documentation/quartz-2.1.7/tutorials/tutorial-lesson-06.html
  
- - `timezone_id`
  - String
  - [Public Preview] A Java timezone id. The schedule will be resolved using this timezone. This will be combined with the quartz_cron_schedule to determine the schedule. See https://docs.databricks.com/sql/language-manual/sql-ref-syntax-aux-conf-mgmt-set-timezone.html for details.
...
- - `telemetry_export_destinations`
  - Sequence
  - [Public Preview]. See [\_](#appsnametelemetry_export_destinations).
...
- - `env`
  - Sequence
  - See [\_](#appsnameconfigenv).
  
:::
  
  
### apps._name_.config.env
...
- - `app`
  - Map
  - See [\_](#appsnameresourcesapp).
  
- - `database`
  - Map
  - See [\_](#appsnameresourcesdatabase).
...
- - `experiment`
  - Map
  - See [\_](#appsnameresourcesexperiment).
  
- - `genie_space`
  - Map
  - See [\_](#appsnameresourcesgenie_space).
...
- - `postgres`
  - Map
  - See [\_](#appsnameresourcespostgres).
...
- - `uc_securable`
  - Map
  - See [\_](#appsnameresourcesuc_securable).
  
:::
  
  
### apps._name_.resources.app
...
### apps._name_.resources.database
...
### apps._name_.resources.experiment
...
### apps._name_.resources.genie_space
...
### apps._name_.resources.postgres
...
### apps._name_.resources.uc_securable
...
### apps._name_.telemetry_export_destinations
...
- - `unity_catalog`
  - Map
  - Unity Catalog Destinations for OTEL telemetry export. See [\_](#appsnametelemetry_export_destinationsunity_catalog).
...
- - `grants`
  - Sequence
  - See [\_](#catalogsnamegrants).
  
- - `lifecycle`
  - Map
  - See [\_](#catalogsnamelifecycle).
  
- - `managed_encryption_settings`
  - Map
  - Control CMK encryption for managed catalog data. See [\_](#catalogsnamemanaged_encryption_settings).
...
### catalogs._name_.grants.privileges
...
### catalogs._name_.managed_encryption_settings
...
- - `azure_encryption_settings`
  - Map
  - optional Azure settings - only required if an Azure CMK is used. See [\_](#catalogsnamemanaged_encryption_settingsazure_encryption_settings).
...
### catalogs._name_.managed_encryption_settings.azure_encryption_settings
...
- - `driver_node_type_flexibility`
  - Map
  - Flexible node type configuration for the driver node. See [\_](#clustersnamedriver_node_type_flexibility).
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#clustersnamelifecycle).
...
- - `worker_node_type_flexibility`
  - Map
  - Flexible node type configuration for worker nodes. See [\_](#clustersnameworker_node_type_flexibility).
...
### clusters._name_.driver_node_type_flexibility
...
### clusters._name_.worker_node_type_flexibility
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#dashboardsnamelifecycle).
...
- - `database_name`
  - String
  - [Public Preview] The name of the database (in a instance) associated with the catalog.
  
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#database_catalogsnamelifecycle).
...
### database_catalogs._name_.lifecycle
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#database_instancesnamelifecycle).
...
- - `node_count`
  - Integer
  - [Public Preview] The number of nodes in the instance, composed of 1 primary and 0 or more secondaries. Defaults to 1 primary and 0 secondaries. This field is input only, see effective_node_count for the output.
  
- - `parent_instance_ref`
  - Map
  - [Public Preview] The ref of the parent instance. This is only available if the instance is child instance. Input: For specifying the parent instance to create a child instance. Optional. Output: Only populated if provided as input to create a child instance. See [\_](#database_instancesnameparent_instance_ref).
  
- - `permissions`
  - Sequence
  - See [\_](#database_instancesnamepermissions).
  
- - `retention_window_in_days`
  - Integer
  - [Public Preview] The retention window for the instance. This is the time window in days for which the historical data is retained. The default value is 7 days. Valid values are 2 to 35 days.
...
### database_instances._name_.custom_tags
...
- - `key`
  - String
  - [Public Preview] The key of the custom tag.
  
- - `value`
  - String
  - [Public Preview] The value of the custom tag.
  
:::
  
  
### database_instances._name_.lifecycle
...
### database_instances._name_.parent_instance_ref
...
- - `branch_time`
  - String
  - [Public Preview] Branch time of the ref database instance. For a parent ref instance, this is the point in time on the parent instance from which the instance was created. For a child ref instance, this is the point in time on the instance from which the child instance was created. Input: For specifying the point in time to create a child instance. Optional. Output: Only populated if provided as input to create a child instance.
  
- - `lsn`
  - String
  - [Public Preview] User-specified WAL LSN of the ref database instance.  Input: For specifying the WAL LSN to create a child instance. Optional. Output: Only populated if provided as input to create a child instance.
  
- - `name`
  - String
  - [Public Preview] Name of the ref database instance.
  
:::
  
  
### database_instances._name_.permissions
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#experimentsnamelifecycle).
...
- - `encryption_details`
  - Map
  - Encryption options that apply to clients connecting to cloud storage. See [\_](#external_locationsnameencryption_details).
...
- - `file_event_queue`
  - Map
  - See [\_](#external_locationsnamefile_event_queue).
  
- - `grants`
  - Sequence
  - See [\_](#external_locationsnamegrants).
  
- - `lifecycle`
  - Map
  - See [\_](#external_locationsnamelifecycle).
...
### external_locations._name_.encryption_details
...
- - `sse_encryption_details`
  - Map
  - Server-Side Encryption properties for clients communicating with AWS s3. See [\_](#external_locationsnameencryption_detailssse_encryption_details).
...
### external_locations._name_.file_event_queue
...
- - `managed_aqs`
  - Map
  - See [\_](#external_locationsnamefile_event_queuemanaged_aqs).
  
- - `managed_pubsub`
  - Map
  - See [\_](#external_locationsnamefile_event_queuemanaged_pubsub).
  
- - `managed_sqs`
  - Map
  - See [\_](#external_locationsnamefile_event_queuemanaged_sqs).
  
- - `provided_aqs`
  - Map
  - See [\_](#external_locationsnamefile_event_queueprovided_aqs).
  
- - `provided_pubsub`
  - Map
  - See [\_](#external_locationsnamefile_event_queueprovided_pubsub).
  
- - `provided_sqs`
  - Map
  - See [\_](#external_locationsnamefile_event_queueprovided_sqs).
  
:::
  
  
### external_locations._name_.file_event_queue.managed_aqs
...
### external_locations._name_.file_event_queue.managed_pubsub
...
### external_locations._name_.file_event_queue.managed_sqs
...
### external_locations._name_.file_event_queue.provided_aqs
...
### external_locations._name_.file_event_queue.provided_pubsub
...
### external_locations._name_.file_event_queue.provided_sqs
...
### external_locations._name_.grants
...
### external_locations._name_.grants.privileges
...
### external_locations._name_.lifecycle
...
- - `budget_policy_id`
  - String
  - [Public Preview] The id of the user specified budget policy to use for this job. If not specified, a default budget policy may be applied when creating or modifying the job. See `effective_budget_policy_id` for the budget policy used by this workload.
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#jobsnamelifecycle).
...
- - `spec`
  - Map
  - The environment entity used to preserve serverless environment side panel, jobs' environment for non-notebook task, and SDP's environment for classic and serverless pipelines. In this minimal environment spec, only pip and java dependencies are supported. See [\_](#jobsnameenvironmentsspec).
...
- - `sparse_checkout`
  - Map
  - See [\_](#jobsnamegit_sourcesparse_checkout).
...
### jobs._name_.git_source.sparse_checkout
...
- - `driver_node_type_flexibility`
  - Map
  - Flexible node type configuration for the driver node. See [\_](#jobsnamejob_clustersnew_clusterdriver_node_type_flexibility).
...
- - `worker_node_type_flexibility`
  - Map
  - Flexible node type configuration for worker nodes. See [\_](#jobsnamejob_clustersnew_clusterworker_node_type_flexibility).
...
### jobs._name_.job_clusters.new_cluster.driver_node_type_flexibility
...
### jobs._name_.job_clusters.new_cluster.worker_node_type_flexibility
...
- - `alert_task`
  - Map
  - [Public Preview] The task evaluates a Databricks alert and sends notifications to subscribers when the `alert_task` field is present. See [\_](#jobsnametasksalert_task).
  
- - `clean_rooms_notebook_task`
  - Map
  - The task runs a [clean rooms](https://docs.databricks.com/clean-rooms/index.html) notebook when the `clean_rooms_notebook_task` field is present. See [\_](#jobsnametasksclean_rooms_notebook_task).
  
- - `compute`
  - Map
  - [Beta] Task level compute configuration. See [\_](#jobsnametaskscompute).
...
- - `power_bi_task`
  - Map
  - [Public Preview] The task triggers a Power BI semantic model update when the `power_bi_task` field is present. See [\_](#jobsnametaskspower_bi_task).
...
- - `workspace_path`
  - String
  - [Public Preview] The workspace_path is the path to the alert file in the workspace. The path: * must start with "/Workspace" * must be a normalized path. User has to select only one of alert_id or workspace_path to identify the alert.
  
:::
  
  
### jobs._name_.tasks.alert_task.subscribers
...
- - `driver_node_type_flexibility`
  - Map
  - Flexible node type configuration for the driver node. See [\_](#jobsnametasksnew_clusterdriver_node_type_flexibility).
...
- - `worker_node_type_flexibility`
  - Map
  - Flexible node type configuration for worker nodes. See [\_](#jobsnametasksnew_clusterworker_node_type_flexibility).
...
### jobs._name_.tasks.new_cluster.driver_node_type_flexibility
...
### jobs._name_.tasks.new_cluster.worker_node_type_flexibility
...
- - `parameters`
  - Map
  - [Beta] Key/value-map of parameters passed to the pipeline execution. Limited to 10k characters in total.
...
- - `refresh_flow_selection`
  - Sequence
  - [Beta] Flow names to selectively refresh. These are unioned with other selective refresh options (refresh_selection, full_refresh_selection) to determine the final set of flows to refresh.
...
- - `tables`
  - Sequence
  - [Public Preview] The tables to be exported to Power BI. See [\_](#jobsnametaskspower_bi_tasktables).
...
- - `email_notifications`
  - Map
  - Email notification settings. See [\_](#model_serving_endpointsnameemail_notifications).
  
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#model_serving_endpointsnamelifecycle).
...
- - `guardrails`
  - Map
  - [Public Preview] Configuration for AI Guardrails to prevent unwanted data and unsafe data in requests and responses. See [\_](#model_serving_endpointsnameai_gatewayguardrails).
...
### model_serving_endpoints._name_.email_notifications
...
### model_serving_endpoints._name_.lifecycle
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#modelsnamelifecycle).
...
- - `tags`
  - Sequence
  - Additional metadata for registered model. See [\_](#modelsnametags).
  
:::
  
  
### models._name_.lifecycle
...
This resource allows you to create [pipelines](/api/workspace/pipelines/create). For information about pipelines, see [_](/dlt/index.md). For a tutorial that uses the Declarative Automation Bundles template to create a pipeline, see [_](/dev-tools/bundles/pipelines-tutorial.md).
...
- - `environment`
  - Map
  - [Public Preview] Environment specification for this pipeline used to install dependencies. See [\_](#pipelinesnameenvironment).
...
- - `ingestion_definition`
  - Map
  - [Public Preview] The configuration for a managed ingestion pipeline. These settings cannot be used with the 'libraries', 'schema', 'target', or 'catalog' settings. See [\_](#pipelinesnameingestion_definition).
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#pipelinesnamelifecycle).
...
- - `data_staging_options`
  - Map
  - [Beta] (Optional) Location of staged data storage. This is required for migration from Cdc Managed Ingestion Pipeline with Gateway pipeline to Combined Cdc Managed Ingestion Pipeline. If not specified, the volume for staged data will be created in catalog and schema/target specified in the top level pipeline definition. See [\_](#pipelinesnameingestion_definitiondata_staging_options).
  
- - `full_refresh_window`
  - Map
  - [Public Preview] (Optional) A window that specifies a set of time ranges for snapshot queries in CDC. See [\_](#pipelinesnameingestion_definitionfull_refresh_window).
...
- - `objects`
  - Sequence
  - [Public Preview] Required. Settings specifying tables to replicate and the destination for the replicated tables. See [\_](#pipelinesnameingestion_definitionobjects).
...
- - `table_configuration`
  - Map
  - [Public Preview] Configuration settings to control the ingestion of tables. These settings are applied to all tables in the pipeline. See [\_](#pipelinesnameingestion_definitiontable_configuration).
...
### pipelines._name_.ingestion_definition.full_refresh_window.days_of_week
...
- - `report`
  - Map
  - [Public Preview] Select a specific source report. See [\_](#pipelinesnameingestion_definitionobjectsreport).
  
- - `schema`
  - Map
  - [Public Preview] Select all tables from a specific source schema. See [\_](#pipelinesnameingestion_definitionobjectsschema).
...
- - `table_configuration`
  - Map
  - [Public Preview] Configuration settings to control the ingestion of tables. These settings override the table_configuration defined in the IngestionPipelineDefinition object. See [\_](#pipelinesnameingestion_definitionobjectsreporttable_configuration).
...
- - `query_based_connector_config`
  - Map
  - [Public Preview] Configurations that are only applicable for query-based ingestion connectors. See [\_](#pipelinesnameingestion_definitionobjectsreporttable_configurationquery_based_connector_config).
...
### pipelines._name_.ingestion_definition.objects.report.table_configuration.auto_full_refresh_policy
...
### pipelines._name_.ingestion_definition.objects.report.table_configuration.query_based_connector_config
...
- - `connector_options`
  - Map
  - [Public Preview] (Optional) Source Specific Connector Options. See [\_](#pipelinesnameingestion_definitionobjectsschemaconnector_options).
...
- - `table_configuration`
  - Map
  - [Public Preview] Configuration settings to control the ingestion of tables. These settings are applied to all tables in this schema and override the table_configuration defined in the IngestionPipelineDefinition object. See [\_](#pipelinesnameingestion_definitionobjectsschematable_configuration).
  
:::
  
  
### pipelines._name_.ingestion_definition.objects.schema.connector_options
...
- - `jira_options`
  - Map
  - [Beta] Jira specific options for ingestion. See [\_](#pipelinesnameingestion_definitionobjectsschemaconnector_optionsjira_options).
  
- - `meta_ads_options`
  - Map
  - [Beta] Meta Marketing (Meta Ads) specific options for ingestion. See [\_](#pipelinesnameingestion_definitionobjectsschemaconnector_optionsmeta_ads_options).
  
:::
  
  
### pipelines._name_.ingestion_definition.objects.schema.connector_options.confluence_options
...
### pipelines._name_.ingestion_definition.objects.schema.connector_options.jira_options
...
### pipelines._name_.ingestion_definition.objects.schema.connector_options.meta_ads_options
...
- - `custom_insights_lookback_window`
  - Integer
  - [Beta] (Optional) Window in days to revisit data during sync to capture updated conversion data from the API.
...
- - `start_date`
  - String
  - [Beta] (Optional) Start date in yyyy-MM-dd format (e.g. 2025-01-15). Data added after this date will be ingested
...
- - `query_based_connector_config`
  - Map
  - [Public Preview] Configurations that are only applicable for query-based ingestion connectors. See [\_](#pipelinesnameingestion_definitionobjectsschematable_configurationquery_based_connector_config).
...
### pipelines._name_.ingestion_definition.objects.schema.table_configuration.auto_full_refresh_policy
...
### pipelines._name_.ingestion_definition.objects.schema.table_configuration.query_based_connector_config
...
- - `table_configuration`
  - Map
  - [Public Preview] Configuration settings to control the ingestion of tables. These settings override the table_configuration defined in the IngestionPipelineDefinition object and the SchemaSpec. See [\_](#pipelinesnameingestion_definitionobjectstabletable_configuration).
  
:::
  
  
### pipelines._name_.ingestion_definition.objects.table.connector_options
...
### pipelines._name_.ingestion_definition.objects.table.connector_options.confluence_options
...
### pipelines._name_.ingestion_definition.objects.table.connector_options.jira_options
...
### pipelines._name_.ingestion_definition.objects.table.connector_options.meta_ads_options
...
- - `query_based_connector_config`
  - Map
  - [Public Preview] Configurations that are only applicable for query-based ingestion connectors. See [\_](#pipelinesnameingestion_definitionobjectstabletable_configurationquery_based_connector_config).
...
### pipelines._name_.ingestion_definition.objects.table.table_configuration.auto_full_refresh_policy
...
### pipelines._name_.ingestion_definition.objects.table.table_configuration.query_based_connector_config
...
### pipelines._name_.ingestion_definition.source_configurations.catalog
...
- - `postgres`
  - Map
  - [Public Preview] Postgres-specific catalog-level configuration parameters. See [\_](#pipelinesnameingestion_definitionsource_configurationscatalogpostgres).
...
### pipelines._name_.ingestion_definition.source_configurations.catalog.postgres
...
- - `slot_config`
  - Map
  - [Public Preview] Optional. The Postgres slot configuration to use for logical replication. See [\_](#pipelinesnameingestion_definitionsource_configurationscatalogpostgresslot_config).
  
:::
  
  
### pipelines._name_.ingestion_definition.source_configurations.catalog.postgres.slot_config
...
- - `auto_full_refresh_policy`
  - Map
  - [Public Preview] (Optional, Mutable) Policy for auto full refresh, if enabled pipeline will automatically try to fix issues by doing a full refresh on the table in the retry run. auto_full_refresh_policy in table configuration will override the above level auto_full_refresh_policy. For example, { "auto_full_refresh_policy": { "enabled": true, "min_interval_hours": 23, } } If unspecified, auto full refresh is disabled. See [\_](#pipelinesnameingestion_definitiontable_configurationauto_full_refresh_policy).
...
- - `query_based_connector_config`
  - Map
  - [Public Preview] Configurations that are only applicable for query-based ingestion connectors. See [\_](#pipelinesnameingestion_definitiontable_configurationquery_based_connector_config).
...
### pipelines._name_.ingestion_definition.table_configuration.auto_full_refresh_policy
...
### pipelines._name_.ingestion_definition.table_configuration.query_based_connector_config
...
- - `glob`
  - Map
  - [Public Preview] The unified field to include source codes. Each entry can be a notebook path, a file path, or a folder path that ends `/**`. This field cannot be used together with `notebook` or `file`. See [\_](#pipelinesnamelibrariesglob).
...
### pipelines._name_.libraries.glob
...
- - `lifecycle`
  - Map
  - See [\_](#postgres_branchesnamelifecycle).
...
### postgres_branches._name_.lifecycle
...
- - `lifecycle`
  - Map
  - See [\_](#postgres_catalogsnamelifecycle).
...
### postgres_catalogs._name_.lifecycle
...
- - `group`
  - Map
  - See [\_](#postgres_endpointsnamegroup).
  
- - `lifecycle`
  - Map
  - See [\_](#postgres_endpointsnamelifecycle).
...
- - `settings`
  - Map
  - A collection of settings for a compute endpoint. See [\_](#postgres_endpointsnamesettings).
...
### postgres_endpoints._name_.group
...
- - `enable_readable_secondaries`
  - Boolean
  - [Beta] Whether to allow read-only connections to read-write endpoints. Only relevant for read-write endpoints where size.max > 1.
  
- - `max`
  - Integer
  - [Beta] The maximum number of computes in the endpoint group. Currently, this must be equal to min. Set to 1 for single compute endpoints, to disable HA. To manually suspend all computes in an endpoint group, set disabled to true on the EndpointSpec.
  
- - `min`
  - Integer
  - [Beta] The minimum number of computes in the endpoint group. Currently, this must be equal to max. This must be greater than or equal to 1.
  
:::
  
  
### postgres_endpoints._name_.lifecycle
...
### postgres_endpoints._name_.settings
...
- - `pg_settings`
  - Map
  - [Beta] A raw representation of Postgres settings.
...
- - `custom_tags`
  - Sequence
  - See [\_](#postgres_projectsnamecustom_tags).
...
- - `default_endpoint_settings`
  - Map
  - A collection of settings for a compute endpoint. See [\_](#postgres_projectsnamedefault_endpoint_settings).
...
- - `lifecycle`
  - Map
  - See [\_](#postgres_projectsnamelifecycle).
  
- - `permissions`
  - Sequence
  - See [\_](#postgres_projectsnamepermissions).
...
### postgres_projects._name_.custom_tags
...
### postgres_projects._name_.default_endpoint_settings
...
- - `autoscaling_limit_max_cu`
  - Any
  - [Beta] The maximum number of Compute Units. Minimum value is 0.5.
  
- - `autoscaling_limit_min_cu`
  - Any
  - [Beta] The minimum number of Compute Units. Minimum value is 0.5.
  
- - `no_suspension`
  - Boolean
  - [Beta] When set to true, explicitly disables automatic suspension (never suspend). Should be set to true when provided. Mutually exclusive with `suspend_timeout_duration`. When updating, use `spec.project_default_settings.suspension` in the update_mask.
...
- - `suspend_timeout_duration`
  - String
  - [Beta] Duration of inactivity after which the compute endpoint is automatically suspended. If specified should be between 60s and 604800s (1 minute to 1 week). Mutually exclusive with `no_suspension`. When updating, use `spec.project_default_settings.suspension` in the update_mask.
  
:::
  
  
### postgres_projects._name_.lifecycle
...
### postgres_projects._name_.permissions
...
- - `lifecycle`
  - Map
  - See [\_](#postgres_synced_tablesnamelifecycle).
  
- - `new_pipeline_spec`
  - Map
  - See [\_](#postgres_synced_tablesnamenew_pipeline_spec).
...
### postgres_synced_tables._name_.lifecycle
...
### postgres_synced_tables._name_.new_pipeline_spec
...
- - `storage_catalog`
  - String
  - [Beta] UC catalog for the pipeline to store intermediate files (checkpoints, event logs etc). This needs to be a standard catalog where the user has permissions to create Delta tables.
  
- - `storage_schema`
  - String
  - [Beta] UC schema for the pipeline to store intermediate files (checkpoints, event logs etc). This needs to be in the standard catalog where the user has permissions to create Delta tables.
...
- - `custom_metrics`
  - Sequence
  - [Create:OPT Update:OPT] Custom metrics. See [\_](#quality_monitorsnamecustom_metrics).
  
- - `inference_log`
  - Map
  - See [\_](#quality_monitorsnameinference_log).
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#quality_monitorsnamelifecycle).
  
- - `notifications`
  - Map
  - [Create:OPT Update:OPT] Field for specifying notification settings. See [\_](#quality_monitorsnamenotifications).
...
- - `schedule`
  - Map
  - [Create:OPT Update:OPT] The monitor schedule. See [\_](#quality_monitorsnameschedule).
...
### quality_monitors._name_.lifecycle
...
- - `on_failure`
  - Map
  - Destinations to send notifications on failure/timeout. See [\_](#quality_monitorsnamenotificationson_failure).
...
- - `aliases`
  - Sequence
  - See [\_](#registered_modelsnamealiases).
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#registered_modelsnamelifecycle).
...
### registered_models._name_.aliases
...
### registered_models._name_.grants.privileges
...
### registered_models._name_.lifecycle
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#schemasnamelifecycle).
...
### schemas._name_.grants.privileges
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#secret_scopesnamelifecycle).
...
### secret_scopes._name_.keyvault_metadata
...
### secret_scopes._name_.lifecycle
...
sql_warehouses:
  <sql_warehouse-name>:
    <sql_warehouse-field-name>: <sql_warehouse-field-value>
```
...
- - `channel`
  - Map
  - Channel Details. See [\_](#sql_warehousesnamechannel).
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#sql_warehousesnamelifecycle).
...
- - `permissions`
  - Sequence
  - See [\_](#sql_warehousesnamepermissions).
...
- - `tags`
  - Map
  - A set of key-value pairs that will be tagged on all resources (e.g., AWS instances and EBS volumes) associated with this SQL warehouse.  Supported values: - Number of tags < 45. See [\_](#sql_warehousesnametags).
...
### sql_warehouses._name_.lifecycle
...
### sql_warehouses._name_.permissions
...
- - `custom_tags`
  - Sequence
  - See [\_](#sql_warehousesnametagscustom_tags).
  
:::
  
  
### sql_warehouses._name_.tags.custom_tags
...
- - `lifecycle`
  - Map
  - See [\_](#synced_database_tablesnamelifecycle).
...
- - `spec`
  - Map
  - [Public Preview]. See [\_](#synced_database_tablesnamespec).
  
:::
  
  
### synced_database_tables._name_.lifecycle
...
### synced_database_tables._name_.spec
...
- - `existing_pipeline_id`
  - String
  - [Public Preview] At most one of existing_pipeline_id and new_pipeline_spec should be defined.  If existing_pipeline_id is defined, the synced table will be bin packed into the existing pipeline referenced. This avoids creating a new pipeline and allows sharing existing compute. In this case, the scheduling_policy of this synced table must match the scheduling policy of the existing pipeline.
  
- - `new_pipeline_spec`
  - Map
  - [Public Preview] At most one of existing_pipeline_id and new_pipeline_spec should be defined.  If new_pipeline_spec is defined, a new pipeline is created for this synced table. The location pointed to is used to store intermediate files (checkpoints, event logs etc). The caller must have write permissions to create Delta tables in the specified catalog and schema. Again, note this requires write permissions, whereas the source table only requires read permissions. See [\_](#synced_database_tablesnamespecnew_pipeline_spec).
...
### synced_database_tables._name_.spec.new_pipeline_spec
...
- - `storage_catalog`
  - String
  - [Public Preview] This field needs to be specified if the destination catalog is a managed postgres catalog.  UC catalog for the pipeline to store intermediate files (checkpoints, event logs etc). This needs to be a standard catalog where the user has permissions to create Delta tables.
  
- - `storage_schema`
  - String
  - [Public Preview] This field needs to be specified if the destination catalog is a managed postgres catalog.  UC schema for the pipeline to store intermediate files (checkpoints, event logs etc). This needs to be in the standard catalog where the user has permissions to create Delta tables.
...
- - `lifecycle`
  - Map
  - See [\_](#vector_search_endpointsnamelifecycle).
...
### vector_search_endpoints._name_.lifecycle
...
### vector_search_endpoints._name_.permissions
...
- - `delta_sync_index_spec`
  - Map
  - See [\_](#vector_search_indexesnamedelta_sync_index_spec).
  
- - `direct_access_index_spec`
  - Map
  - See [\_](#vector_search_indexesnamedirect_access_index_spec).
...
- - `grants`
  - Sequence
  - See [\_](#vector_search_indexesnamegrants).
...
- - `lifecycle`
  - Map
  - See [\_](#vector_search_indexesnamelifecycle).
...
### vector_search_indexes._name_.delta_sync_index_spec
...
### vector_search_indexes._name_.delta_sync_index_spec.embedding_source_columns
...
### vector_search_indexes._name_.delta_sync_index_spec.embedding_vector_columns
...
### vector_search_indexes._name_.direct_access_index_spec
...
- - `embedding_vector_columns`
  - Sequence
  - See [\_](#vector_search_indexesnamedirect_access_index_specembedding_vector_columns).
...
### vector_search_indexes._name_.direct_access_index_spec.embedding_source_columns
...
### vector_search_indexes._name_.direct_access_index_spec.embedding_vector_columns
...
### vector_search_indexes._name_.grants
...
### vector_search_indexes._name_.grants.privileges
...
### vector_search_indexes._name_.lifecycle
...
- - `lifecycle`
  - Map
  - Lifecycle is a struct that contains the lifecycle settings for a resource. It controls the behavior of the resource when it is deployed or destroyed. See [\_](#volumesnamelifecycle).
...
### volumes._name_.grants.privileges
