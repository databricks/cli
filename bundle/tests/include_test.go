...
func TestIncludeWithGlob(t *testing.T) {
	...
	assert.Equal(t, "include_with_glob/job.yml", filepath.ToSlash(job.DynamicValue.Location().File))
}
...
func TestIncludeForMultipleMatches(t *testing.T) {
	...
	assert.Equal(t, "include_multiple/my_first_job/resource.yml", filepath.ToSlash(first.DynamicValue.Location().File))

	second := b.Config.Resources.Jobs["my_second_job"]
	assert.Equal(t, "2", second.ID)
	assert.Equal(t, "include_multiple/my_second_job/resource.yml", filepath.ToSlash(second.DynamicValue.Location().File))
}
