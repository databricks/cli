...
func TestSyncIncludeExcludeNoMatchesTest(t *testing.T) {
	...
	require.Equal(t, diags[1].Location.File, filepath.Join("override_sync", "databricks.yml"))
	...
	require.Equal(t, diags[2].Location.File, filepath.Join("override_sync", "databricks.yml"))
