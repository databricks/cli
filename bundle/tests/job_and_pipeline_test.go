...
func TestJobAndPipelineDevelopment(t *testing.T) {
	...
	assert.Equal(t, "job_and_pipeline/databricks.yml", filepath.ToSlash(p.LocalConfigFilePath))
...
func TestJobAndPipelineProduction(t *testing.T) {
	...
	assert.Equal(t, "job_and_pipeline/databricks.yml", filepath.ToSlash(j.LocalConfigFilePath))
