...
func TestJobAndPipelineDevelopmentWithEnvironment(t *testing.T) {
	...
	assert.Equal(t, "environments_job_and_pipeline/databricks.yml", filepath.ToSlash(p.DynamicValue.Location().File))
...
func TestJobAndPipelineProductionWithEnvironment(t *testing.T) {
	...
	assert.Equal(t, "environments_job_and_pipeline/databricks.yml", filepath.ToSlash(j.DynamicValue.Location().File))
