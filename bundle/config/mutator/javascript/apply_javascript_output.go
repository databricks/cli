...
// applyJavaScriptOutputResult contains which resources where added, updated, or deleted by JavaScript mutator.
type applyJavaScriptOutputResult struct {
...
// applyJavaScriptOutput applies output of JavaScript mutator to bundle configuration before JavaScript mutator.
//
// It records applyJavaScriptOutputResult containing which resources where added, updated, or deleted
// by JavaScript mutator.
//
...
func applyJavaScriptOutput(root, output dyn.Value) (dyn.Value, applyJavaScriptOutputResult, error) {
...
func createOverrideVisitor(leftRoot, rightRoot dyn.Value) (applyJavaScriptOutputResult, merge.OverrideVisitor) {
	...
	return applyJavaScriptOutputResult{
...
func isOmitemptyDelete(left dyn.Value) bool {
	// JavaScript output can omit empty sequences/mappings, because we don't track them as optional,
	// there is no semantic difference between empty and missing, so we keep them as they were before
	// JavaScript mutator deleted them.
