...
func (m *processRootIncludes) Apply(ctx context.Context, b *bundle.Bundle) diag.Diagnostics {
	...
	for _, extraIncludePath := range getExtraIncludePaths(ctx) {
		if filepath.IsAbs(extraIncludePath) {
			rel, err := filepath.Rel(b.Path, extraIncludePath)
			if err != nil {
	...
	for _, entry := range b.Config.Include {
		...
		matches, err := filepath.Glob(filepath.Join(b.Path, entry))
		if err != nil {
		...
		for _, match := range matches {
			rel, err := filepath.Rel(b.Path, match)
			if err != nil {
				return diag.FromErr(err)
			}
			if _, ok := seen[rel]; ok {
		...
		for _, include := range includes {
			out = append(out, ProcessInclude(filepath.Join(b.Path, include), include))
		}
	}
