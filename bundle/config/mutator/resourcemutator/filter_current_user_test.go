...
func TestEnsureCurrentUserPermissions(t *testing.T) {
	b := testFixture("alice@databricks.com")

	diags := bundle.Apply(context.Background(), b, FilterCurrentUser())
	assert.NoError(t, diags.Error())

	// Assert current user has IS_OWNER permission for jobs (other permissions removed)
	assert.Len(t, b.Config.Resources.Jobs["job1"].Permissions, 3)
	...
	assert.Contains(t, b.Config.Resources.Jobs["job1"].Permissions, resources.JobPermission{Level: "IS_OWNER", UserName: "alice@databricks.com"})

	assert.Len(t, b.Config.Resources.Jobs["job2"].Permissions, 3)
	...
	assert.Contains(t, b.Config.Resources.Jobs["job2"].Permissions, resources.JobPermission{Level: "IS_OWNER", UserName: "alice@databricks.com"})

	// Assert current user has IS_OWNER permission for pipelines
	assert.Len(t, b.Config.Resources.Pipelines["pipeline1"].Permissions, 3)
	...
	assert.Contains(t, b.Config.Resources.Pipelines["pipeline1"].Permissions, resources.PipelinePermission{Level: "IS_OWNER", UserName: "alice@databricks.com"})

	// Assert current user has CAN_MANAGE permission for other resources
	assert.Len(t, b.Config.Resources.Experiments["experiment1"].Permissions, 3)
	...
	assert.Contains(t, b.Config.Resources.Experiments["experiment1"].Permissions, resources.MlflowExperimentPermission{Level: "CAN_MANAGE", UserName: "alice@databricks.com"})

	assert.Len(t, b.Config.Resources.Models["model1"].Permissions, 3)
	...
	assert.Contains(t, b.Config.Resources.Models["model1"].Permissions, resources.MlflowModelPermission{Level: "CAN_MANAGE", UserName: "alice@databricks.com"})

	assert.Len(t, b.Config.Resources.ModelServingEndpoints["endpoint1"].Permissions, 3)
	...
	assert.Contains(t, b.Config.Resources.ModelServingEndpoints["endpoint1"].Permissions, resources.ModelServingEndpointPermission{Level: "CAN_MANAGE", UserName: "alice@databricks.com"})

	// Assert there's no change to the grant (registered models use grants, not permissions).
	assert.Len(t, b.Config.Resources.RegisteredModels["registered_model1"].Grants, 1)
}

func TestEnsureCurrentServicePrincipalPermissions(t *testing.T) {
	b := testFixture("i-Robot")

	diags := bundle.Apply(context.Background(), b, FilterCurrentUser())
	assert.NoError(t, diags.Error())

	// Assert current service principal has IS_OWNER permission for jobs
	assert.Len(t, b.Config.Resources.Jobs["job1"].Permissions, 3)
	assert.Contains(t, b.Config.Resources.Jobs["job1"].Permissions, jobAlice)
	assert.Contains(t, b.Config.Resources.Jobs["job1"].Permissions, jobBob)
	assert.Contains(t, b.Config.Resources.Jobs["job1"].Permissions, resources.JobPermission{Level: "IS_OWNER", ServicePrincipalName: "i-Robot"})

	assert.Len(t, b.Config.Resources.Jobs["job2"].Permissions, 3)
	assert.Contains(t, b.Config.Resources.Jobs["job2"].Permissions, jobAlice)
	assert.Contains(t, b.Config.Resources.Jobs["job2"].Permissions, jobBob)
	assert.Contains(t, b.Config.Resources.Jobs["job2"].Permissions, resources.JobPermission{Level: "IS_OWNER", ServicePrincipalName: "i-Robot"})

	// Assert current service principal has IS_OWNER permission for pipelines
	assert.Len(t, b.Config.Resources.Pipelines["pipeline1"].Permissions, 3)
	assert.Contains(t, b.Config.Resources.Pipelines["pipeline1"].Permissions, pipelineAlice)
	assert.Contains(t, b.Config.Resources.Pipelines["pipeline1"].Permissions, pipelineBob)
	assert.Contains(t, b.Config.Resources.Pipelines["pipeline1"].Permissions, resources.PipelinePermission{Level: "IS_OWNER", ServicePrincipalName: "i-Robot"})

	// Assert current service principal has CAN_MANAGE permission for other resources
	assert.Len(t, b.Config.Resources.Experiments["experiment1"].Permissions, 3)
	assert.Contains(t, b.Config.Resources.Experiments["experiment1"].Permissions, experimentAlice)
	assert.Contains(t, b.Config.Resources.Experiments["experiment1"].Permissions, experimentBob)
	assert.Contains(t, b.Config.Resources.Experiments["experiment1"].Permissions, resources.MlflowExperimentPermission{Level: "CAN_MANAGE", ServicePrincipalName: "i-Robot"})

	assert.Len(t, b.Config.Resources.Models["model1"].Permissions, 3)
	assert.Contains(t, b.Config.Resources.Models["model1"].Permissions, modelAlice)
	assert.Contains(t, b.Config.Resources.Models["model1"].Permissions, modelBob)
	assert.Contains(t, b.Config.Resources.Models["model1"].Permissions, resources.MlflowModelPermission{Level: "CAN_MANAGE", ServicePrincipalName: "i-Robot"})

	assert.Len(t, b.Config.Resources.ModelServingEndpoints["endpoint1"].Permissions, 3)
	assert.Contains(t, b.Config.Resources.ModelServingEndpoints["endpoint1"].Permissions, endpointAlice)
	assert.Contains(t, b.Config.Resources.ModelServingEndpoints["endpoint1"].Permissions, endpointBob)
	assert.Contains(t, b.Config.Resources.ModelServingEndpoints["endpoint1"].Permissions, resources.ModelServingEndpointPermission{Level: "CAN_MANAGE", ServicePrincipalName: "i-Robot"})
