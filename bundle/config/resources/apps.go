...
type App struct {
	...
	// the Create/Update App request so they are recorded on the app. source_code_path points
	// at app source on local disk and is translated to its workspace path. The API accepts
	// both on write but does not echo them on read (input_only), so drift is suppressed via
	// ignore_remote_changes. They are also passed to the Deploy API, which requires a
	// deployment source. Declaring them here too would duplicate their json tags and break diffing.
