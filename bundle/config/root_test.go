...
func TestRootMergeTargetOverridesWithVariableDefaultClearsLookup(t *testing.T) {
	root := &Root{
		Variables: map[string]*variable.Variable{
			"foo": {
	...
	require.NoError(t, root.MergeTargetOverrides("development"))
	assert.Equal(t, "bar", root.Variables["foo"].Default)
	assert.Nil(t, root.Variables["foo"].Lookup)
}

func TestRootMergeTargetOverridesWithVariableLookupClearsDefault(t *testing.T) {
	root := &Root{
		Variables: map[string]*variable.Variable{
			"foo": {
				Default: "bar",
	...
	require.NoError(t, root.initializeDynamicValue())
	require.NoError(t, root.MergeTargetOverrides("development"))
	assert.Nil(t, root.Variables["foo"].Default)
	assert.Equal(t, "my-cluster", root.Variables["foo"].Lookup.Cluster)
}

func TestRootMergeTargetOverridesWithShorthandVariableOverrideClearsLookup(t *testing.T) {
	root, diags := LoadFromBytes("databricks.yml", []byte(`
variables:
  my_variable:
    lookup:
...
`))
	...
	require.NoError(t, root.MergeTargetOverrides("my-target"))
	assert.Equal(t, "foobar", root.Variables["my_variable"].Default)
	assert.Nil(t, root.Variables["my_variable"].Lookup)
}
