// Command generate writes bundle/terraform_dabs_map/generated.go with the
// precomputed DABs↔Terraform field mapping. Run from the repository root.
package main
...
type groupResult struct {
	...
	matchCount int               // count of exactly-matched fields
}
...
func main() {
	...
	for _, group := range groups {
		...
		if err != nil {
			log.Fatalf("build group %s: %v", group, err)
		}
	...
	const outPath = "bundle/terraform_dabs_map/generated.go"
...
func renderSource(results []groupResult) ([]byte, error) {
	...
	w("// Code generated by bundle/terraform_dabs_map/cmd/generate; DO NOT EDIT.\n\n")
	w("package terraform_dabs_map\n\n")

	w("// TerraformToDABsFieldMap maps DABs group name → (TF field path → DABs field path)\n")
	w("// for fields that exist in both but under different names. Exact matches are omitted.\n")
	...
	w("// MatchCounts maps DABs group name → count of exactly-matched fields.\n")
	w("var MatchCounts = map[string]int{\n")
	for _, r := range results {
		if !r.hasTFType {
			continue
		}
		w("\t%q: %d,\n", r.group, r.matchCount)
	}
	w("}\n")
