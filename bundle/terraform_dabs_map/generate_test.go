...
func renderSource(results []groupResult) ([]byte, error) {
	...
	w("// Summary (renames: DABs↔TF leaf-name differences; dabs-only/tf-only: unmatched fields; unwraps: spec wrappers stripped):\n")
	for _, r := range results {
		if r.hasTFType {
			w("//   %s (%s): %d renames, %d dabs-only, %d tf-only, %d unwraps\n",
				r.group, r.tfType, len(r.renames), len(r.dabsOnly), len(r.tfOnly), len(r.unwraps))
		}
	}
	w("\n")
