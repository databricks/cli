...
func SafeError(err error) string {
	if se, ok := errors.AsType[*safeErr](err); ok {
		return se.safeErr
	}

	// Not raised through this package, but a typed error can still describe
	// itself — libs/filer's errors do. That covers the call sites not converted.
...
func SafeSprintf(format string, args ...any) string {
	safeFormat, safeArgs, ok := escapeUnsafeVerbs(format, args)
	if !ok {
		...
		return truncateSafe(format + " (safeerr failed)")
	}
	...
	return truncateSafe(fmt.Errorf(safeFormat, safeArgs...).Error())
}
...
func escapeUnsafeVerbs(format string, args []any) (string, []any, bool) {
	...
	for i := 0; i < len(format); {
		...
		// More verbs than arguments is a malformed call, which go vet reports at
		// the call site; here the surplus verb simply has nothing to render.
		var value any
		safeToRender := false
		if argIndex < len(args) {
			value, safeToRender = safeValueFor(args[argIndex])
		}
		argIndex++
	...
	return sb.String(), safeArgs, true
}
...
// safeValueFor returns what to render for arg, and whether rendering it is safe
// at all. Everything else is dropped, so a value the caller did not vouch for
// cannot reach the output.
func safeValueFor(arg any) (any, bool) {
	if v, ok := arg.(safeValue); ok {
		// A value that declares a stand-in keeps it even under Safe: the type
		// knows which part of itself is user data, so it outranks a call-site
		// assertion that the whole value is safe.
		if ss, ok := v.v.(SafeStringer); ok {
			return standIn(ss), true
		}
		return v.v, true
	}

	// A nested safe error contributes its own safe message. Only the error itself
	// is consulted, not its chain: an inner safe message is not the wrapper's own
	// text, and walking the chain would not terminate when Unwrap loops back.
	if se, ok := arg.(*safeErr); ok {
		return safeErrProxy{msg: se.safeErr}, true
	}

	// Failing that, a typed error can still describe itself, which is how
	// libs/filer reports a classification without the path it carries.
