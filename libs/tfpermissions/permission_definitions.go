...
import (
	...
	"github.com/databricks/cli/libs/tfpermissions/read"
	"github.com/databricks/cli/libs/tfpermissions/update"
...
func (p resourcePermissions) ObjectType() string { return p.objectType }
...
func GetResourcePermissionsFromId(id string) (resourcePermissions, error) {
	...
	if len(idParts) < 2 {
		return resourcePermissions{}, fmt.Errorf("cannot parse object id: %q", id)
	}
	objectType := strings.Join(idParts[1:len(idParts)-1], "/")
	for _, mapping := range allResourcePermissions() {
		// fmt.Fprintf(os.Stderr, "mapping.requestObjectType=%q objectType=%q id=%q\n", mapping.requestObjectType, objectType, id)
...
func allResourcePermissions() []resourcePermissions {
	...
	return []resourcePermissions{
		...
		},
		{
			...
			requestObjectType: "database-instances",
			allowedPermissionLevels: map[string]permissionLevelOptions{
				"CAN_USE":    {isManagementPermission: false},
				"CAN_MANAGE": {isManagementPermission: true},
			},
