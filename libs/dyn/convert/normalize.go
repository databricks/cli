...
func Normalize(dst any, src dyn.Value, field_substitutions map[string]string, opts ...NormalizeOption) (dyn.Value, diag.Diagnostics) {
	...
	return n.normalizeType(reflect.TypeOf(dst), src, []reflect.Type{}, field_substitutions)
}

func (n normalizeOptions) normalizeType(typ reflect.Type, src dyn.Value, seen []reflect.Type, field_substitutions map[string]string) (dyn.Value, diag.Diagnostics) {
	...
	switch typ.Kind() {
	case reflect.Struct:
		return n.normalizeStruct(typ, src, append(seen, typ), field_substitutions)
	case reflect.Map:
		return n.normalizeMap(typ, src, append(seen, typ), field_substitutions)
	case reflect.Slice:
		return n.normalizeSlice(typ, src, append(seen, typ), field_substitutions)
	case reflect.String:
...
func (n normalizeOptions) normalizeStruct(typ reflect.Type, src dyn.Value, seen []reflect.Type, field_substitutions map[string]string) (dyn.Value, diag.Diagnostics) {
	...
	case dyn.KindMap:
		...
		for k, v := range src.MustMap() {
			...
			v, err := n.normalizeType(typ.FieldByIndex(index).Type, v, seen, field_substitutions)
			if err != nil {
		...
		for k, index := range info.Fields {
			...
			case reflect.Struct, reflect.Map:
				v, _ = n.normalizeType(ftyp, dyn.V(map[string]dyn.Value{}), seen, field_substitutions)
			case reflect.Slice:
				v, _ = n.normalizeType(ftyp, dyn.V([]dyn.Value{}), seen, field_substitutions)
			case reflect.String:
				v, _ = n.normalizeType(ftyp, dyn.V(""), seen, field_substitutions)
			case reflect.Bool:
				v, _ = n.normalizeType(ftyp, dyn.V(false), seen, field_substitutions)
			case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
				v, _ = n.normalizeType(ftyp, dyn.V(int64(0)), seen, field_substitutions)
			case reflect.Float32, reflect.Float64:
				v, _ = n.normalizeType(ftyp, dyn.V(float64(0)), seen, field_substitutions)
			default:
...
func (n normalizeOptions) normalizeMap(typ reflect.Type, src dyn.Value, seen []reflect.Type, field_substitutions map[string]string) (dyn.Value, diag.Diagnostics) {
	var diags diag.Diagnostics
	...
	case dyn.KindMap:
		...
		for k, v := range src.MustMap() {
			...
			v, err := n.normalizeType(typ.Elem(), v, seen, field_substitutions)
			if err != nil {
				diags = diags.Extend(err)
				// Skip the element if it cannot be normalized.
				if !v.IsValid() {
...
func (n normalizeOptions) normalizeSlice(typ reflect.Type, src dyn.Value, seen []reflect.Type, field_substitutions map[string]string) (dyn.Value, diag.Diagnostics) {
	var diags diag.Diagnostics
