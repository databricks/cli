...
func (d *loader) loadSequence(node *yaml.Node, loc dyn.Location) (dyn.Value, error) {
	...
	return dyn.NewValue(acc, dyn.Locations{loc}), nil
}
...
func (d *loader) loadMapping(node *yaml.Node, loc dyn.Location) (dyn.Value, error) {
	...
	return dyn.NewValue(out, dyn.Locations{loc}), nil
}
...
func (d *loader) loadScalar(node *yaml.Node, loc dyn.Location) (dyn.Value, error) {
	...
	switch st {
	case "!!str":
		return dyn.NewValue(node.Value, dyn.Locations{loc}), nil
	case "!!bool":
		switch strings.ToLower(node.Value) {
		case "true":
			return dyn.NewValue(true, dyn.Locations{loc}), nil
		case "false":
			return dyn.NewValue(false, dyn.Locations{loc}), nil
		default:
	...
	case "!!int":
		...
		if i64 >= math.MinInt32 && i64 <= math.MaxInt32 {
			return dyn.NewValue(int(i64), dyn.Locations{loc}), nil
		}
		return dyn.NewValue(i64, dyn.Locations{loc}), nil
	case "!!float":
		f64, err := strconv.ParseFloat(node.Value, 64)
		if err != nil {
			return dyn.InvalidValue, errorf(loc, "invalid float value: %v", node.Value)
		}
		return dyn.NewValue(f64, dyn.Locations{loc}), nil
	case "!!null":
		return dyn.NewValue(nil, dyn.Locations{loc}), nil
	case "!!timestamp":
		...
		for _, layout := range []string{
			...
			if terr == nil {
				return dyn.NewValue(t, dyn.Locations{loc}), nil
			}
		}
