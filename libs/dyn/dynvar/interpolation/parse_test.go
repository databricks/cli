...
func TestParseEscapeDollar(t *testing.T) {
	...
	t.Run("escaped_ref", func(t *testing.T) {
		tokens, err := Parse("$${a}")
		require.NoError(t, err)
		assert.Equal(t, []Token{
			{Kind: TokenLiteral, Value: "${a}", Start: 0, End: 5},
		}, tokens)
	})

	t.Run("double_escape", func(t *testing.T) {
		tokens, err := Parse("$$$$")
		require.NoError(t, err)
		assert.Equal(t, []Token{
	...
	t.Run("escape_then_ref", func(t *testing.T) {
		tokens, err := Parse("$$$$${a.b}")
		require.NoError(t, err)
		assert.Equal(t, []Token{
			{Kind: TokenLiteral, Value: "$$", Start: 0, End: 4},
...
func TestParseNestedReference(t *testing.T) {
	// Nested ${...} inside a reference: the outer ${ is treated as literal,
	// and the inner reference is parsed normally.
	tokens, err := Parse("${var.foo_${var.tail}}")
	require.NoError(t, err)
	assert.Equal(t, []Token{
...
func TestParsePositions(t *testing.T) {
	...
	t.Run("literal_ref_literal", func(t *testing.T) {
		tokens, err := Parse("pre ${a.b} post")
		require.NoError(t, err)
		assert.Equal(t, []Token{
			{Kind: TokenLiteral, Value: "pre ", Start: 0, End: 4},
	...
	t.Run("adjacent_refs", func(t *testing.T) {
		tokens, err := Parse("${a}${b}")
		require.NoError(t, err)
		assert.Equal(t, []Token{
			{Kind: TokenRef, Value: "a", Start: 0, End: 4},
	...
	t.Run("dollar_sign_mid_literal", func(t *testing.T) {
		tokens, err := Parse("a$b")
		require.NoError(t, err)
		assert.Equal(t, []Token{
			{Kind: TokenLiteral, Value: "a$b", Start: 0, End: 3},
		}, tokens)
	})

	t.Run("escape_between_refs", func(t *testing.T) {
		tokens, err := Parse("${a}$$${b}")
		require.NoError(t, err)
		assert.Equal(t, []Token{
			{Kind: TokenRef, Value: "a", Start: 0, End: 4},
