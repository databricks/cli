...
func TestMergeKeepsTheWritesStateAndMask(t *testing.T) {
	// A failure claims status, error_message and the id, so the state the write recorded and
	// the mask that carries it survive: the resource stays listed as it was written.
	write, err := NewStateUpdate("id-1", json.RawMessage(`{"state":{"name":"before"}}`), false)
	require.NoError(t, err)
	failed := NewFailureUpdate("id-1", errors.New("boom"))
	...
	assert.Equal(t, "id-1", merged.ResourceID)
