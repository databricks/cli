...
func (cm *connectionsManager) TryAdd(id string, conn *proxyConnection) bool {
	...
	cmdio.LogString(context.Background(), fmt.Sprintf("Client connected, current count: %d", len(cm.connections)))
...
func (cm *connectionsManager) Remove(id string) {
	...
	cmdio.LogString(context.Background(), fmt.Sprintf("Client disconnected, current count: %d", len(cm.connections)))
...
func (cm *connectionsManager) startShutdownTimer() {
	...
	cmdio.LogString(context.Background(), fmt.Sprintf("Starting shutdown timer for %v", cm.shutdownDelay))
	cm.shutdownTimer = time.AfterFunc(cm.shutdownDelay, func() {
		cmdio.LogString(context.Background(), "Shutdown timer expired, shutting down...")
		cm.TimedOut <- true
	})
}
...
func (cm *connectionsManager) cancelShutdownTimer() {
	...
	defer cm.shutdownTimerMu.Unlock()
	if cm.shutdownTimer != nil {
		cmdio.LogString(context.Background(), "Cancelling shutdown timer")
		cm.shutdownTimer.Stop()
	}
}
...
func (handler *sshHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if conn, exists := handler.Connections.Get(id); exists && conn != nil {
		cmdio.LogString(ctx, fmt.Sprintf("Client with id %s is already connected", id))
		err := conn.AcceptHandover(r.Context(), w, r)
		if err != nil {
			cmdio.LogError(ctx, fmt.Errorf("failed to accept handover: %v", err))
			http.Error(w, "Handover failed", http.StatusInternalServerError)
		} else {
			cmdio.LogString(ctx, "Handover accepted for client with id "+id)
		}
		return
	}
	...
	if !handler.Connections.TryAdd(id, proxy) {
		err = proxy.Close()
		if err != nil {
			cmdio.LogError(ctx, fmt.Errorf("failed to close websocket: %v", err))
		}
		return
	}
	defer func() {
		cmdio.LogString(ctx, "Closing WebSocket connection")
	...
	cmdio.LogString(ctx, "New WebSocket connection, starting sshd process")
	...
	if err != nil {
		cmdio.LogError(ctx, fmt.Errorf("failed to get stdin pipe: %v", err))
		return
	}
	...
	if err != nil {
		cmdio.LogError(ctx, fmt.Errorf("failed to get stdout pipe: %v", err))
		return
	}
	...
	if err := g.Wait(); err != nil {
		cmdio.LogError(ctx, fmt.Errorf("SSHHandler error: %v", err))
	}
}

func handleTimeout(ctx context.Context, timedOutChannel chan bool, shutdownDelay time.Duration) {
	<-timedOutChannel
	cmdio.LogString(ctx, fmt.Sprintf("No SSH clients for %v, shutting down...", shutdownDelay))
	os.Exit(0)
}
...
func saveWorkspaceMetadata(ctx context.Context, client *databricks.WorkspaceClient, version, clusterID string, port int) error {
	...
	cmdio.LogString(ctx, fmt.Sprintf("Saved port %d to metadata file: %s", port, metadataPath))
	return nil
}
...
func saveJupyterInitScript(ctx context.Context) error {
	...
	cmdio.LogString(ctx, "Saved Jupyter init script to: "+initScriptPath)
	return nil
}
...
func RunServer(ctx context.Context, client *databricks.WorkspaceClient, opts ServerOptions) error {
	...
	cmdio.LogString(ctx, "Starting server on "+listenAddr)
