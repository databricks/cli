...
Go serialization/deserialization for Databricks metric-view YAML definitions
(versions 0.1, 1.0, and 1.1; single- and multi-source). It mirrors the YAML
shapes accepted by the metric-view serde implementation in the backend.

## Scope

Pure serde. No semantic validation (the backend validates when a
...
`testdata/fixtures/*.yaml` exercise every feature of the format. They use
