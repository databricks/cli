...
type BundleConfigRemoteSyncEvent struct {
	...
	// is an opaque random UUIDv4 minted by dstate.GetOrInitLineage (uuid.New(),
	// so no MAC address or timestamp) and is never derived from a user,
	// workspace, path, or resource name. For terraform-engine bundles the value
	// is generated by Terraform itself and only parsed through.
