...
func TestMultipartLimiterBoundsConcurrency(t *testing.T) {
	...
	_, err := e.upload(t.Context(), "/Volumes/c/s/v/m.bin", bytes.NewReader(in),
...
func TestSingleShotAcquiresOneSlot(t *testing.T) {
	...
	_, err := e.upload(t.Context(), "/Volumes/c/s/v/s.bin", bytes.NewReader(in), WithLimiter(lim))
	noErr(t, err)
	eqBytes(t, f.assembled(), in)
...
func TestResumableAcquiresPerChunk(t *testing.T) {
	...
	_, err := e.upload(t.Context(), "/Volumes/c/s/v/r.bin", bytes.NewReader(in), WithLimiter(lim))
	noErr(t, err)
	eqBytes(t, f.assembled(), in)
...
func TestLimiterCancelNoLeak(t *testing.T) {
	...
	go func() {
		_, err := e.upload(ctx, "/Volumes/c/s/v/c.bin", bytes.NewReader(data(16*1024)),
			WithParallelism(8), WithLimiter(lim))
		errc <- err
	}()
