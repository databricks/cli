...
// isDetected returns true if an agent has been detected in the context.
// Returns false if Detect() hasn't been called yet.
func isDetected(ctx context.Context) bool {
	v := ctx.Value(productKey)
	if v == nil {
		return false
	}
