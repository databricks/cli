...
replace existing fields, and inject a curated optional from INJECT that the base omits. Free-form
scalars are occasionally replaced with dangerous / near-range-end values (empty, whitespace,
over-long, control characters, int32/int64 boundaries) to probe the CLI's input handling.
...
A failure is a real CLI bug (panic, internal error, or drift); the failing seed's `LOG.repro`
prints a ready-to-run repro, of the form
