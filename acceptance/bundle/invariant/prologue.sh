...
# On success it marks `deployed` (so cleanup destroys) and prints INPUT_CONFIG_OK, after
# which the fuzzer treats any failure as a bug.
invariant_deploy() {
    set +e
    trace "$@" &> LOG.deploy
    deploy_rc=$?
