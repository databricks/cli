...
the update request instead of being cleared (see #6343). Configs without `# CAN_REMOVE` only run the
baseline. If a field's removal does not converge, tag it `# CANNOT_REMOVE` with a comment explaining the
error or drift, rather than leaving a failing `# CAN_REMOVE`. The marker is matched with its `# ` prefix,
so `# CANNOT_REMOVE` and prose that mentions the marker are neither stripped nor treated as a tag.
