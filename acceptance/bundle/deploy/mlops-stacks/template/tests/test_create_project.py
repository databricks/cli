...
def assert_no_disallowed_strings_in_files(file_paths, disallowed_strings, exclude_path_matches=None):
    """
    ...
    def should_check_file_for_disallowed_strings(path):
        return not any(substring in path for substring in exclude_path_matches) and os.path.isfile(path)
...
def test_no_template_strings_after_param_substitution(cloud, include_models_in_unity_catalog, generated_project_dir):
    ...
    assert_no_disallowed_strings_in_files(
        file_paths=[os.path.join(generated_project_dir, path) for path in paths(generated_project_dir)],
        disallowed_strings=["{{", "{%", "%}"],
...
def test_generate_project_with_default_values(
    ...
    test_file_contents = (tmpdir / TEST_PROJECT_NAME / "_params_testing_only.txt").read_text("utf-8")
    if cloud == "azure":
...
def prepareContext(
    ...
    if include_models_in_unity_catalog != "":
        context["input_include_models_in_unity_catalog"] = include_models_in_unity_catalog
    return context
...
def test_generate_project_check_delta_output(
    ...
    generate(tmpdir, databricks_cli, context=context)
    delta_notebook_path = tmpdir / TEST_PROJECT_NAME / TEST_PROJECT_DIRECTORY / "training" / "notebooks" / "Train.py"
    if setup_cicd_and_project != "CICD_Only" and include_mlflow_recipes == "no" and include_feature_store == "no":
...
def test_generate_project_check_recipe_output(
    ...
    recipe_notebook_path = (
        tmpdir / TEST_PROJECT_NAME / TEST_PROJECT_DIRECTORY / "training" / "notebooks" / "TrainWithMLflowRecipes.py"
    )
