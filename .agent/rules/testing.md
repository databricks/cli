...
**RULE: `EnvMatrix.<VAR> = []` removes that variable from the inherited matrix** (see `ExpandEnvMatrix` in `acceptance/internal/config.go`). The `out.test.toml` snapshot of inherited values is generated and committed by design.

**RULE: For `DATABRICKS_BUNDLE_ENGINE` specifically, do NOT opt out with `= []`.** The root `test.toml` matrixes `["terraform", "direct"]`, and CI splits work per-engine via `ENVFILTER=DATABRICKS_BUNDLE_ENGINE=<value>`. An empty list runs the test *once* locally with the variable unset, but on CI the test has no engine tag, so `checkEnvFilters` lets it through on BOTH the direct and terraform runners — duplicating the run. Pin to a single value instead: `EnvMatrix.DATABRICKS_BUNDLE_ENGINE = ["direct"]`. If the test needs to exercise the default (unset) engine, `unset DATABRICKS_BUNDLE_ENGINE` at the top of the script. `validateConfig` in `acceptance/internal/config.go` enforces this; `selftest/` is exempt because it exercises the mechanic itself.
