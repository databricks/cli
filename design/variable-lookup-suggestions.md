...
Core fuzzy matching (`Levenshtein`, `ClosestMatch`) lives in `libs/textutil/`
and is independent of the `dyn` package. The `SuggestPath` function in
`libs/dyn/dynvar/` uses these functions with dyn tree walking.
