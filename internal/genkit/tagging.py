...
Thin wrapper around ``tagging_upstream.py`` — which is regenerated verbatim from
universe (``openapi/tagging/tagging.py``) by ``genkit update-sdk`` and must stay
pristine. Despite the filename, THIS file is hand-maintained: the tagging
workflow runs ``tagging.py`` (this wrapper), which holds the CLI-specific
behavior instead of editing the synced file:
...
import tagging_upstream as tagging
