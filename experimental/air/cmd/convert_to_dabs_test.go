...
`
	...
	assert.Equal(t, "./"+codeSourceTarballName, get(t, root, art+".code_source_path").MustString())
	...
	// command.sh is always an artifact; inline deps synthesize requirements.yaml.
	assert.Contains(t, itemNames(artifacts), commandScriptName)
	assert.Contains(t, itemNames(artifacts), requirementsName)
}
...
// A git-pinned code_source is archived at the commit, not the dirty working tree:
// writeBundle produces code_source.tar.gz containing the committed file only.
func TestConvertToDabsGitPinnedTarball(t *testing.T) {
	repo := newTestRepo(t)
	writeRepoFile(t, repo, "train.py", "print('committed')")
	sha := commitAll(t, repo, "init")
	// Dirty the tree AFTER the commit; the pinned archive must not include this.
	...
	assert.Contains(t, entries, dir+"/train.py")
	assert.NotContains(t, entries, dir+"/uncommitted.py", "git-pinned archive must exclude uncommitted files")
}

// git archive runs with `git -C repoPath`, so a relative output dir must be
// resolved to absolute before packaging — otherwise `-o out/...` resolves against
// the repo dir and fails. Exercise writeBundle from a working dir with a RELATIVE
// output path against a git-pinned source.
func TestConvertToDabsGitPinnedRelativeOutputDir(t *testing.T) {
	...
	assert.Contains(t, entries, filepath.Base(repo)+"/train.py")
}
