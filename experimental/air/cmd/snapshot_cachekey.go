...
// SHA-256 over every file's path, size, mtime, mode, and symlink target (sorted for stability). An unchanged
// tree yields the same key, so an already-uploaded tarball can be reused instead of
// re-packaged and re-uploaded. Regular-file content detection is size+mtime, not content —
// the same trade-off DABs file-sync makes — so an edit preserving both is not seen.
