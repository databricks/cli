...
// writeBundleProject renders databricks.yml (via the exportbundle.go converter) plus
// the command.sh and code the bundle syncs, into a temp bundle root. Returns the
...
// from cfg.Command and a targets block; wiring code_source snapshotting through the
// inherited bundle/config/mutator/aicode mutator is the remaining piece.
func writeBundleProject(cfg *runConfig, configPath string) (string, func(), error) {
	...
	bundleDoc := convertToBundle(cfg)
	body, err := marshalBundle(bundleDoc, configPath)
	if err != nil {
		cleanup()
		return "", func() {}, err
	}
	...
	body = append(body, []byte(bundleTargetsBlock())...)
	if err := os.WriteFile(filepath.Join(root, "databricks.yml"), body, 0o600); err != nil {
		cleanup()
		return "", func() {}, err
	}
