...
const (
	...
	// `tar`. Content-addressed by the working-tree metadata fingerprint, so an
	// unchanged tree reuses the uploaded tarball; a same-size, same-mtime regular-file edit is missed.
	modePlainTar
)
