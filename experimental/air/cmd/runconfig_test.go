...
// The polymorphic union types decode from a scalar or a sequence, so they must
// report their YAML shape rather than the struct reflection sees.
func TestResolveConfigField_PolymorphicTypes(t *testing.T) {
	tests := []struct{ path, wantType string }{
		{"config.environment.dependencies", "string (path) or list of strings"},
