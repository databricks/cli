...
func TestResolveNodeCount(t *testing.T) {
	...
	// A run with no AIR compute errors.
	_, err := resolveNodeCount(&jobs.Run{RunId: 1})
	require.Error(t, err)
}

// downloadServer serves the full MLflow artifact chain: artifacts/list (logs
// dir + per-node chunks), credentials-for-read (a pre-signed URL back at itself),
// and the chunk bytes.
...
// fullDownloadServer adds runs/get + get-output to downloadServer so the whole
// downloadLogs entry point can run.
func fullDownloadServer(t *testing.T, numGPUs int) *httptest.Server {
	t.Helper()
	var base string
	runGet := `{
		"run_id": 123,
		"state": {"life_cycle_state": "TERMINATED", "result_state": "SUCCESS"},
		"tasks": [{"run_id": 456, "ai_runtime_task": {"deployments": [{"compute": {"accelerator_type": "GPU_1xA10", "accelerator_count": ` + strconv.Itoa(numGPUs) + `}}]}}]
	}`
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
