...
func Run(ctx context.Context, client *databricks.WorkspaceClient, opts ServerOptions) error {
	...
	// itself exec'd from /Workspace. Best-effort: the tunnel has always run
	// without a registration of its own, so a node whose FUSE daemons do not
	// answer must not lose the tunnel as well. Mirrors the seedEnvActivation
