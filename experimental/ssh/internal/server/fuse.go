...
// registerFuseCredentials registers this server process with the node's WSFS and
// UC-FUSE daemons, so that /Workspace, /Volumes and /dbfs keep working for the
// server and for every session under it once the bootstrap notebook's Python
// REPL — today the only registered ancestor — is gone.
//
// Called for its side effect on the daemons; the registration is renewed in the
...
func registerFuseCredentials(ctx context.Context, client *databricks.WorkspaceClient) error {
	...
	// The user id is only a tag on the registration, for audit logging, so a
	// workspace that will not answer must not stop the registration itself.
	userID := ""
...
// authenticate with. It goes through Authenticate rather than reading
// Config.Token so that every credential type the CLI supports works, and so that
// a refreshed OAuth token is picked up on each call.
func workspaceToken(client *databricks.WorkspaceClient) fuse.TokenFunc {
	return func(ctx context.Context) (string, error) {
		...
		if !ok {
			// The FUSE daemons take a bearer token and nothing else, so anything
			// but a Bearer scheme has to be reported rather than passed on.
