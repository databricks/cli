...
func TestParsePidNamespace(t *testing.T) {
	tests := []struct {
		...
		// A uint32 field, so an id that does not fit must fail, not truncate.
...
func TestSelfDescribesTheRunningProcess(t *testing.T) {
	...
	assert.NotZero(t, self.PidNamespaceID, "a zero namespace id is rejected")
	assert.NotZero(t, self.StartTime, "a zero start time is rejected")
}
...
func TestRegisterSendsBothDaemonsThePidAndCredentials(t *testing.T) {
	...
	// The pid goes in the path, not the body.
...
func TestRegisterRejectsWhatTheDaemonsWouldMisread(t *testing.T) {
	...
	// An empty token clears the registration, so registering one would remove the
	// access it was meant to grant.
	err := client.Register(t.Context(), r, "", "")
	assert.ErrorContains(t, err, "empty token")

	// Rejected by the daemons; catching it here makes the error name the cause.
...
func TestRevokeClearsTheRegistration(t *testing.T) {
	...
	// A revoke is an empty token with a zero start time.
...
func TestOneDaemonFailingStillRegistersTheOther(t *testing.T) {
	...
	// One of the two being registered is better than neither, so both are asked
	// and the errors are reported together.
