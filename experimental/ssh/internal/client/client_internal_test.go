...
func TestResolveAndValidateBaseEnvironment(t *testing.T) {
	ctx := cmdio.MockDiscard(t.Context())
	...
	t.Run("env.yaml path passthrough", func(t *testing.T) {
		m := mocks.NewMockWorkspaceClient(t)
		got, err := resolveAndValidateBaseEnvironment(ctx, m.WorkspaceClient, "/Workspace/path/to/env.yaml")
	...
	t.Run("display name v4 resolves to resource ID", func(t *testing.T) {
		m := mocks.NewMockWorkspaceClient(t)
		listReturns(m, []environments.WorkspaceBaseEnvironment{
			{DisplayName: "other", Name: "workspace-base-environments/dbe_other", Spec: specV4},
			{DisplayName: "my-env", Name: "workspace-base-environments/dbe_mine", Spec: specV4},
		}, nil)
		got, err := resolveAndValidateBaseEnvironment(ctx, m.WorkspaceClient, "my-env")
		require.NoError(t, err)
	...
	t.Run("display name ambiguous", func(t *testing.T) {
		m := mocks.NewMockWorkspaceClient(t)
		listReturns(m, []environments.WorkspaceBaseEnvironment{
			{DisplayName: "my-env", Name: "workspace-base-environments/dbe_1", Spec: specV4},
			{DisplayName: "my-env", Name: "workspace-base-environments/dbe_2", Spec: specV4},
		}, nil)
		_, err := resolveAndValidateBaseEnvironment(ctx, m.WorkspaceClient, "my-env")
		require.Error(t, err)
