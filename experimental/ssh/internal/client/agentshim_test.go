...
// gatewayServer replies with v2Status at the v2 endpoints API and v3Status at the
// v3 model-services API, 404ing anything else.
func gatewayServer(t *testing.T, v2Status, v3Status int) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/api/ai-gateway/v2/endpoints":
			assert.Equal(t, "Bearer test-token", r.Header.Get("Authorization"))
			w.WriteHeader(v2Status)
		case "/api/2.1/unity-catalog/model-services":
			assert.Equal(t, "Bearer test-token", r.Header.Get("Authorization"))
			w.WriteHeader(v3Status)
		default:
			// Other paths (e.g. the SDK's /.well-known/databricks-config discovery) 404.
			w.WriteHeader(http.StatusNotFound)
		}
		_, _ = w.Write([]byte("{}"))
	}))
...
func TestProbeAIGateway(t *testing.T) {
	t.Run("enabled via v2", func(t *testing.T) {
		srv := gatewayServer(t, http.StatusOK, http.StatusNotFound)
		defer srv.Close()
		assert.NoError(t, probeAIGateway(t.Context(), newProbeClient(t, srv.URL)))
	})

	t.Run("enabled via v3 fallback", func(t *testing.T) {
		srv := gatewayServer(t, http.StatusNotFound, http.StatusOK)
		defer srv.Close()
		assert.NoError(t, probeAIGateway(t.Context(), newProbeClient(t, srv.URL)))
	})

	t.Run("disabled returns an actionable error", func(t *testing.T) {
		srv := gatewayServer(t, http.StatusForbidden, http.StatusForbidden)
		defer srv.Close()
