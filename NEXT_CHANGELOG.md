...
* Fix `bundle config-remote-sync --select-ids` failing the entire sync with "no deployed <type> resource with id" when one selected resource was deleted remotely or its deploy state drifted; such selectors are now skipped so the remaining resources still sync ([#XXXX](https://github.com/databricks/cli/pull/XXXX)).
