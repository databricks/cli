...
* `apps init` no longer emits duplicate variables, resource entries, and env vars when two selected features declare the same resource (e.g. `--features=database,lakebase`, which share a postgres resource) ([#6623](https://github.com/databricks/cli/pull/6623)).
