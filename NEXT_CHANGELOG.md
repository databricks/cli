...
* Added experimental OS-native secure token storage opt-in via `DATABRICKS_AUTH_STORAGE=secure` (or `[__settings__] auth_storage = secure`). The default `plaintext` mode keeps the existing file-backed token cache with host-key dual-write.
