...
### CLI
* Ctrl+C in an interactive prompt now prints `cancelled` and exits 130 instead of `Error: user aborted` / `Error: ^C` and exit 1. Applies to both the `huh` prompts in `databricks aitools` and the bubbletea-based prompts in `libs/cmdio`.
