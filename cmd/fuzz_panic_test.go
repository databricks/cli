...
// perInvocationTimeout bounds a single Execute call. Some auto-generated
// commands poll long-running operations (e.g. apps create-space) that would
// otherwise hang against our stub server.
...
func isAutoGenerated(leaf leafCommand) bool {
	...
	manualRoots := map[string]bool{
		"bundle": true, "auth": true, "sync": true, "fs": true, "api": true,
		"cache": true, "completion": true, "configure": true, "experimental": true,
		"labs": true, "pipelines": true, "psql": true, "selftest": true, "ssh": true,
...
// progressPrinter prints "[i/total] path" to stderr before each leaf, so a
// long fuzz run shows how many leaves are left.
...
func newProgressPrinter(label string, total int) *progressPrinter {
	fmt.Fprintf(os.Stderr, "%s: %d leaves to fuzz\n", label, total)
	return &progressPrinter{total: total, start: time.Now()}
}

func (p *progressPrinter) tick(leaf leafCommand) {
	p.i++
	fmt.Fprintf(os.Stderr, "[%d/%d %s] %s\n", p.i, p.total, time.Since(p.start).Round(time.Millisecond), leafPathName(leaf))
}
...
func TestCountFuzz(t *testing.T) {
	...
	progress := newProgressPrinter("count-fuzz", len(leaves))
