...
The server communicates via stdio using the Model Context Protocol.`,
  ...
  databricks mcp --warehouse-id abc123

  # Start with custom Docker image for validation
  databricks mcp --warehouse-id abc123 --docker-image node:20-alpine

  # Start with deployment tools enabled
  databricks mcp --warehouse-id abc123 --allow-deployment

  # Start without containerized validation
  databricks mcp --warehouse-id abc123 --use-dagger=false`,
