...
package account_access_control_proxy
...
var Cmd = &cobra.Command{
	Use:   "account-access-control-proxy",
...
var getAssignableRolesForResourceCmd = &cobra.Command{
  ...
  role.`,
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		response, err := w.AccountAccessControlProxy.GetAssignableRolesForResource(ctx, getAssignableRolesForResourceReq)
		if err != nil {
			return err
		}
...
var getRuleSetCmd = &cobra.Command{
  ...
  rule set for each resource is supported.`,
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		response, err := w.AccountAccessControlProxy.GetRuleSet(ctx, getRuleSetReq)
		if err != nil {
			return err
		}
		return cmdio.Render(ctx, response)
...
var updateRuleSetCmd = &cobra.Command{
  ...
  Replace the rules of a rule set. First, use a GET rule set request to read the
  current version of the rule set before modifying it. This pattern helps
  prevent conflicts between concurrent updates.`,
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		response, err := w.AccountAccessControlProxy.UpdateRuleSet(ctx, updateRuleSetReq)
		if err != nil {
			return err
		}
		return cmdio.Render(ctx, response)
...
// end service AccountAccessControlProxy
