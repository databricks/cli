...
func newCreate() *cobra.Command {
	...
	cmd.Flags().BoolVar(&createReq.AllowDuplicateNames, "allow-duplicate-names", createReq.AllowDuplicateNames, `If false, deployment will fail if name conflicts with that of another pipeline. Wire name: 'allow_duplicate_names'.`)
	cmd.Flags().StringVar(&createReq.BudgetPolicyId, "budget-policy-id", createReq.BudgetPolicyId, `Budget policy of this pipeline. Wire name: 'budget_policy_id'.`)
	cmd.Flags().StringVar(&createReq.Catalog, "catalog", createReq.Catalog, `A catalog in Unity Catalog to publish data from this pipeline to. Wire name: 'catalog'.`)
	cmd.Flags().StringVar(&createReq.Channel, "channel", createReq.Channel, `DLT Release Channel that specifies which version to use. Wire name: 'channel'.`)
	// TODO: array: clusters
	// TODO: map via StringToStringVar: configuration
	cmd.Flags().BoolVar(&createReq.Continuous, "continuous", createReq.Continuous, `Whether the pipeline is continuous or triggered. Wire name: 'continuous'.`)
	// TODO: complex arg: deployment
	cmd.Flags().BoolVar(&createReq.Development, "development", createReq.Development, `Whether the pipeline is in Development mode. Wire name: 'development'.`)
	cmd.Flags().BoolVar(&createReq.DryRun, "dry-run", createReq.DryRun, `Wire name: 'dry_run'.`)
	cmd.Flags().StringVar(&createReq.Edition, "edition", createReq.Edition, `Pipeline product edition. Wire name: 'edition'.`)
	// TODO: complex arg: environment
	// TODO: complex arg: event_log
	// TODO: complex arg: filters
	// TODO: complex arg: gateway_definition
	cmd.Flags().StringVar(&createReq.Id, "id", createReq.Id, `Unique identifier for this pipeline. Wire name: 'id'.`)
	// TODO: complex arg: ingestion_definition
	// TODO: array: libraries
	cmd.Flags().StringVar(&createReq.Name, "name", createReq.Name, `Friendly identifier for this pipeline. Wire name: 'name'.`)
	// TODO: array: notifications
	cmd.Flags().BoolVar(&createReq.Photon, "photon", createReq.Photon, `Whether Photon is enabled for this pipeline. Wire name: 'photon'.`)
	// TODO: complex arg: restart_window
	cmd.Flags().StringVar(&createReq.RootPath, "root-path", createReq.RootPath, `Root path for this pipeline. Wire name: 'root_path'.`)
	// TODO: complex arg: run_as
	cmd.Flags().StringVar(&createReq.Schema, "schema", createReq.Schema, `The default schema (database) where tables are read from or published to. Wire name: 'schema'.`)
	cmd.Flags().BoolVar(&createReq.Serverless, "serverless", createReq.Serverless, `Whether serverless compute is enabled for this pipeline. Wire name: 'serverless'.`)
	cmd.Flags().StringVar(&createReq.Storage, "storage", createReq.Storage, `DBFS root directory for storing checkpoints and tables. Wire name: 'storage'.`)
	// TODO: map via StringToStringVar: tags
	cmd.Flags().StringVar(&createReq.Target, "target", createReq.Target, `Target schema (database) to add tables in this pipeline to. Wire name: 'target'.`)
