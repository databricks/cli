...
func newDelete() *cobra.Command {
  ...
  the request must be retried by using the fresh etag in the 409 response.`
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) (err error) {
		...
		response, err := w.RestrictWorkspaceAdmins.Delete(ctx, deleteReq)
		if err != nil {
			return err
		}
...
func newGet() *cobra.Command {
  ...
  Gets the restrict workspace admins setting.`
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) (err error) {
		...
		response, err := w.RestrictWorkspaceAdmins.Get(ctx, getReq)
		if err != nil {
			return err
		}
		return cmdio.Render(ctx, response)
...
func newUpdate() *cobra.Command {
  ...
  must be retried by using the fresh etag in the 409 response.`
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) (err error) {
		...
		response, err := w.RestrictWorkspaceAdmins.Update(ctx, updateReq)
		if err != nil {
			return err
		}
		return cmdio.Render(ctx, response)
