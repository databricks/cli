  ...
  Arguments:
	...
	cmd.Args = func(cmd *cobra.Command, args []string) error {
		if cmd.Flags().Changed("json") {
			...
			if err != nil {
				return fmt.Errorf("when --json flag is specified, provide only PARENT, RESOURCE_KEY as positional arguments. Provide 'action_type', 'resource_id', 'status' in your JSON input")
			}
			return nil
		}
		check := root.ExactArgs(1)
