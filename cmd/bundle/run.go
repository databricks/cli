...
`,
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		// Non-inline: use ProcessBundleRet for full lifecycle including state management.
		// PostInitFunc resolves the argument and handles scripts (no state needed).
		// PostStateFunc handles resource runs (state already opened by process.go).
		var key string
		var runArgs []string
		...
		_, _, err := utils.ProcessBundleRet(cmd, utils.ProcessOptions{
			...
			PostInitFunc: func(ctx context.Context, b *bundle.Bundle) error {
				...
				// Script path: execute immediately (no state needed).
				// executeScript replaces the process on success or returns error.
