...
func newBindCommand() *cobra.Command {
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		if !exists {
			return fmt.Errorf("%s with an id '%s' is not found", resource.ResourceType(), args[1])
		}
		...
		cmdio.LogString(ctx, fmt.Sprintf("Successfully bound %s with an id '%s'. Run 'bundle deploy' to deploy changes to your workspace", resource.ResourceType(), args[1]))
		return nil
	}
