...
Spec. This include the Databricks Go, Python and Java SDKs. For more information
...
value, you'll be prompted for values at runtime.

While this command always logs you into the specified host, the runtime behavior
depends on the existing profiles you have setup in your configuration file
(at ~/.databrickscfg by default).

1. If a profile with the specified name exists, and it specifies a host, you'll
...
2. If a profile with the specified name exists, but it does not specify a host.
...
3. If a profile with the specified name exists, and it specifies a host, but you
