...
func TestDockerTokenEmitsGetResponse(t *testing.T) {
	...
	loadToken := func(_ context.Context, args TokenRequest) (*oauth2.Token, error) {
		gotProfile = args.Profile.Name
...
func TestRunDockerTokenUsesMatchedProfileAccountID(t *testing.T) {
	...
	err := runDockerToken(ctx, cmd, tokenOptions{
		profiler: profiler,
	}, func(_ context.Context, args TokenRequest) (*oauth2.Token, error) {
		assert.Equal(t, "profile-account", args.Profile.AccountID)
		return &oauth2.Token{AccessToken: "access-token"}, nil
	})
	require.NoError(t, err)
}
...
func TestDockerTokenRejectsPositionalArgs(t *testing.T) {
	...
	cmd := newDockerTokenCommand(func(context.Context, TokenRequest) (*oauth2.Token, error) {
...
func TestDockerTokenRejectsAuthSelectionFlags(t *testing.T) {
	...
	for _, args := range cases {
		t.Run(strings.Join(args, " "), func(t *testing.T) {
			...
			dockerCmd.AddCommand(newDockerTokenCommand(func(context.Context, TokenRequest) (*oauth2.Token, error) {
				t.Fatal("loadToken should not be called")
				return nil, nil
			}))
