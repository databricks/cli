...
func init() {
	...
	deleteCmd.Flags().StringVar(&deleteReq.Id, "id", "", `Unique ID for a service principal in the Databricks Workspace.`)
...
func init() {
	...
	getCmd.Flags().StringVar(&getReq.Id, "id", "", `Unique ID for a service principal in the Databricks Workspace.`)
