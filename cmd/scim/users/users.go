...
var deleteCmd = &cobra.Command{
  ...
  objects associated with the user.`,
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		w := sdk.WorkspaceClient(ctx)
		if len(args) == 0 {
			names, err := w.Users.UserUserNameToIdMap(ctx, scim.ListUsersRequest{})
			if err != nil {
				return err
			}
			id, err := ui.PromptValue(cmd.InOrStdin(), names, "Unique ID for a user in the Databricks Workspace")
			if err != nil {
				return err
			}
			args = append(args, id)
