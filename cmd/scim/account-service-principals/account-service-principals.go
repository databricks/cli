...
var deleteCmd = &cobra.Command{
  ...
  Delete a single service principal in the Databricks Account.`,
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		a := sdk.AccountClient(ctx)
		if len(args) == 0 {
			names, err := a.ServicePrincipals.ServicePrincipalDisplayNameToIdMap(ctx, scim.ListServicePrincipalsRequest{})
			if err != nil {
				return err
			}
			id, err := ui.PromptValue(cmd.InOrStdin(), names, "Unique ID for a service principal in the Databricks Account")
			if err != nil {
				return err
			}
			args = append(args, id)
