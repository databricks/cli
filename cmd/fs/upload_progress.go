...
func formatPlainProgress(p files.Progress) string {
	if p.Total <= 0 {
...
func (r *progressRenderer) render(now time.Time, p files.Progress) string {
...
func newProgressFunc(ctx context.Context) (files.ProgressFunc, func()) {
	if cmdio.GetInteractiveMode(ctx) == cmdio.InteractiveModeNone {
		nextPct := 10
		fn := func(p files.Progress) {
