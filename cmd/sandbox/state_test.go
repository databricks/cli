...
// operations are isolated from the developer's real ~/.databricks/sandbox.json.
func stateCtx(t *testing.T) (context.Context, string) {
	t.Helper()
	home := t.TempDir()
	ctx := env.WithUserHomeDir(t.Context(), home)
	return ctx, filepath.Join(home, ".databricks", "sandbox.json")
}
...
func TestStateSetGetDefaultRoundTrip(t *testing.T) {
	...
	require.NoError(t, setDefault(ctx, "profile-a", "sandbox-a"))
	assert.Equal(t, "sandbox-a", getDefault(ctx, "profile-a"))
...
func TestStateMultipleProfilesIndependent(t *testing.T) {
	...
	require.NoError(t, setDefault(ctx, "profile-a", "sandbox-a"))
	require.NoError(t, setDefault(ctx, "profile-b", "sandbox-b"))

	assert.Equal(t, "sandbox-a", getDefault(ctx, "profile-a"))
	assert.Equal(t, "sandbox-b", getDefault(ctx, "profile-b"))
}
...
func TestStateSetDefaultOverwrites(t *testing.T) {
	...
	require.NoError(t, setDefault(ctx, "profile-a", "sandbox-a"))
	require.NoError(t, setDefault(ctx, "profile-a", "sandbox-a-prime"))
	assert.Equal(t, "sandbox-a-prime", getDefault(ctx, "profile-a"))
}
