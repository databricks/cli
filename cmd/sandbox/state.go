...
// stateFile stores per-profile sandbox state on the local filesystem.
// Located at ~/.databricks/sandbox.json.
type stateFile struct {
	// Profile name → default sandbox ID.
	...
	// name-based resolution (`sandbox ssh my-project` finds the matching
	// sandbox ID without an extra API call). Refreshed in full by
	// `sandbox list`; mutated in-place by `create`, `config --name`,
...
// stateFilePath returns the on-disk location of the sandbox state file.
func stateFilePath(ctx context.Context) (string, error) {
	home, err := env.UserHomeDir(ctx)
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".databricks", "sandbox.json"), nil
}

// loadState reads the sandbox state file, returning an empty state when
...
func setDefault(ctx context.Context, profile, sandboxID string) error {
	state, err := loadState(ctx)
	if err != nil {
		return err
	}
	if state.Defaults[profile] == sandboxID {
		return nil
	}
	state.Defaults[profile] = sandboxID
...
// `sandbox list` to keep the cache in sync with the server's view.
